...
1. Preflight-Check (`checklists/01-preflight.md`) und `checklists/03-before-code-change.md` sind durchgeführt; Kontrollstufe mit auslösendem Faktor und Modus M3 sind benannt.
...
3. Die Aufgabe verfolgt genau ein Ziel (Q1), hat Akzeptanzkriterien und einen erwarteten Umfang von höchstens `<CHANGE_SIZE_THRESHOLD>` Dateien. Stufe niedrig: eine bereinigte, klare Aufgabenbeschreibung genügt. Ab Stufe mittel: ein bestätigter Plan nach `templates/PLAN_TEMPLATE.md` liegt vor (Bestätigungsstatus „bestätigt" mit Rolle und Datum); der Mensch fügt den Plan in die Aufgabe ein oder referenziert ihn im Arbeitsbereich (`<TBD: Ablage von Plänen im Projekt>`); der Skill liest keine Plan-Dateien außerhalb des Arbeitsbereichs. Die Zieldateien enthalten keine nicht zugeordneten lokalen Änderungen (Prüfung durch den Menschen vor dem Aufruf), damit der Änderungssatz der Sitzung eindeutig der Aufgabe zuzuordnen und einzeln rücknehmbar ist (P7).
4. Freigabevoraussetzungen je Kontrollstufe (wörtlich aus `framework/core/09-risk-model.md` Abschnitt 3); bei Stufe mittel ohne bestätigten Plan oder Stufe hoch ohne dokumentierte Freigabe und begleitende Person wird die Bearbeitung abgelehnt:
...
**Ausgeschlossene Informationen:** K3 gemäß `framework/core/02-privacy.md`; `<EXCLUDED_PATHS>`; `<CI_CONFIG_PATHS>`, `<QUALITY_GATE_CONFIG_PATHS>`, Lockfiles und Manifestdateien der Abhängigkeitsverwaltung als Änderungsziel; Ticket-Kommentarverläufe, Anhänge und Kundenkommunikation; Produktionsdaten und Produktionslogs; Werte aus Konfigurations- und Umgebungsdateien.
...
11. Ergebnis im Ausgabeformat erzeugen, einschließlich Hinweis auf `checklists/04-review-ai-code.md`; Ergebnisbericht gemäß `framework/core/05-working-model.md` Abschnitt 3.6 anhängen.
...
- Diff vollständig lesen; `checklists/04-review-ai-code.md` (niedrig: RV1, RV2, RV5, RV9, RV10; ab mittel RV1–RV12 mit Planabgleich); <TEST_COMMAND> selbst ausführen; Commit erstellen; Merge Request mit Devin-Nutzungsvermerk (fw-mr-description)
```
...
3. `checklists/04-review-ai-code.md` abarbeiten (bei Testanteilen zusätzlich `checklists/05-testing.md`); Annahmen und offene Fragen in den Merge Request übernehmen; Commit nach `<COMMIT_CONVENTION>` erstellen; Übernahme ausschließlich über den bestehenden Review- und Freigabeprozess mit Devin-Nutzungsvermerk (`templates/MR_AI_DISCLOSURE.md`, Skill `fw-mr-description`).
