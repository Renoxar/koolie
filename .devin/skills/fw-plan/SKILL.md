...
- **Zweck:** Erarbeitet vor jeder Modifikation einen umsetzbaren, prüfbaren Änderungsplan exakt nach `templates/PLAN_TEMPLATE.md`: Ziel und Akzeptanzkriterien, Ist-Zustand mit Fundstellen, gekennzeichnete Annahmen und offene Fragen, bewertete Optionen (mindestens zwei bei Stufe mittel und hoch), kleine einzeln prüfbare Schritte mit Prüfung je Schritt, Teststrategie, Risiken und Gegenmaßnahmen, Rollback, Abbruchkriterien und Freigabeerfordernis. Der Plan ist Grundlage der Planbestätigung (Schritt 9 des Standardarbeitsablaufs) und der späteren Umsetzung mit `fw-change-small`, `fw-tests` oder `fw-docs-update`.
...
1. Preflight-Check (`checklists/01-preflight.md`) durchgeführt; Kontrollstufe mit auslösendem Faktor durch den Menschen festgelegt; Modus M2 benannt.
...
**Zulässige Kontextquellen:** Quellcode, Tests und Schnittstellenbeschreibungen in `<ALLOWED_PATHS>` und `<READ_ONLY_PATHS>`; Dokumentation in `<DOC_PATHS>`; Overlay-Dokumente der Klasse K1 laut Manifest (Architektur, Conventions, Definition of Done); Analyseergebnis aus der Sitzung; `templates/PLAN_TEMPLATE.md`.

**Ausgeschlossene Informationen:** K3 gemäß `framework/core/02-privacy.md`; `<EXCLUDED_PATHS>`; Ticket-Kommentare und Anhänge; Werte aus Konfigurations- und Umgebungsdateien; Produktions- und Infrastrukturdetails.
...
### Plan (Struktur exakt nach templates/PLAN_TEMPLATE.md)
...
- Plan aus der Sitzungsausgabe oder ~/.devin/plans/ in Ticket, Merge Request oder Projektablage übernehmen; Umsetzung in neuer Sitzung mit fw-change-small, fw-tests oder fw-docs-update
```
...
3. Plan in Ticket, Merge Request oder Projektablage (`<TBD: Ablage von Plänen im Projekt>`) übernehmen; die Plan-Datei aus `~/.devin/plans/` nicht in das Repository committen.
4. Jede Planänderung nach Bestätigung erfordert eine erneute Bestätigung (`framework/core/05-working-model.md`, M2). Vor dem ersten Umsetzungsschritt `checklists/03-before-code-change.md` abarbeiten.
