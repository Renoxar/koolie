...
Verzeichnissegment hielten** (`CR-2026-122`, **D-298** bis **D-304**, `K-84` und `K-97`
geschlossen, `K-100` neu, **Pruefung 75 und 76**). **Ohne Kontingent, ohne Lauf an
einem Client.**

> 🔴 **Der schwerste Befund ist eine Abhilfe, die genau an ihrem Anlass zerbrochen
> waere.** An `install.py` stand seit `0.02.0` woertlich, `<CORE_DIR>` werde dort
> gesetzt, *"damit eine spaetere Umbenennung nur eine Stelle beruehrt"*. Die Bindung lief
> ueber `os.path.basename()` - und `os.path.basename(".koolie/core")` ist **`core`**.
> **Die Zeile, die eine Umbenennung billig machen sollte, haette sie nicht ueberlebt.**
> 🔴 **Und es waere kein Validatorfehler gewesen:** Der Validator band denselben Wert an
> derselben Stelle auf dieselbe Weise und haette ihn **bestaetigt**. *Zwei Stellen, die
> einander decken* (0.57.0), diesmal ueber Werkzeuggrenzen hinweg.
> ➡️ *Eine Abhilfe, die einen Fall vorwegnimmt, gehoert an dem Fall gemessen, den sie
> vorwegnimmt - nicht an dem, den es gerade gibt.*

### Was sich aendert

- 🟢 **Das Framework heisst `Koolie`.** Der Kern liegt unter **`.koolie/core/`**, das
  Overlay unter **`.koolie/project-overlay/`**; `<CORE_DIR>` traegt damit **erstmals
  einen Schraegstrich** (D-125, D-127, D-272, `CR-2026-098`, `CR-2026-119`).
  **498 verfolgte Dateien** sind mit einem `git mv` gewandert, der Textlauf hat
  **1.645 Ersetzungen in 197 Traegern** geschrieben.
- 🟢 **Die Lage des Kerns wird nicht mehr abgeleitet, sondern benannt** (**D-299**).
  `CORE_REL` steht an **vier** Stellen - `clientmap.py`, dem Validator, dem Schutz-Hook
  und `ablage.py` -, und **Pruefung 76 haelt sie gegeneinander**. Zwei Gegenstaende, und
  sie sind dieselbe Frage aus zwei Richtungen: alle vier tragen denselben Wert, **und
  keine leitet ihn ueber `os.path.basename` ab**. *Ohne den zweiten waere der erste
  erfuellbar, indem alle vier denselben Fehler machen - und genau das war der Zustand
  bis `0.87.0`.*
- 🟢 **Zwei weitere Stellen zaehlten EBENEN und rechnen jetzt aus `CORE_TIEFE`**
  (D-299): `PROJEKTWURZEL` des Schutz-Hooks (vier `dirname`, noetig fuenf) und `WURZEL`
  in `ablage.py` (drei, noetig vier). 🔴 **Die so bestimmte Projektwurzel waere
  `.koolie/` gewesen** - nach dem eigenen Kommentar des Hooks *die obere Freigabegrenze
  der Pfadaufloesung*, also eine Ebene zu tief.
- 🆕 **Pruefung 75 meldet einen Restbestand des alten Namens** (**D-271**, `CR-2026-119`
  E3). *Sonst ist "der Name ist weg" eine Behauptung ohne Pruefung.* **Zaehlbereich ist
  jeder verfolgte Traeger, nicht nur der Kern** - die Lehre von D-295. Ausgenommen ist
  eine **deklarierte** Menge in vier Klassen, **in beide Richtungen geprueft**.
- 🔴 **Die Namen datierter Belegablagen ausserhalb des Repositoriums wandern NICHT mit**
  (**D-300**). `devpacks/leitwerk-erhebungen-<datum>-<buendel>`,
  `leitwerk-UEBERGABE-archiv-...` bleiben, wie sie heissen. *Ein Sweep haette aus jeder
  vorhandenen Belegablage eine gemacht, die es nicht gibt* - **D-283 mit umgekehrtem
  Vorzeichen und selbstverschuldet.** ⚠️ **Preis:** Die Belegablagen dieses Frameworks
  tragen bis auf weiteres den alten Namen; der Wechsel gehoert an einen Messtag, der eine
  neue Reihe beginnt.
- 🔴 **Sieben von 23 blossen Namensnennungen waren Aussagen UEBER den alten Namen**
  (**D-301**) - darunter die Changelog-Zeile zu `0.7.0`, zwei Messangaben, die das
  **gemessene Suchmuster** nennen, und 🔴 **die Namensmetapher der Wurzel-README**
  (*"Ein Leitwerk fliegt das Flugzeug nicht - es haelt es stabil und auf Kurs"*). **Acht
  Stellen sind vor dem Lauf maskiert und danach woertlich zurueckgesetzt worden**, mit
  einem Waechter auf genau einen Treffer. ⚠️ **Preis, ein echter Verlust:** Das Bild
  bleibt, die Namensableitung entfaellt. `Koolie` traegt keine eigene, und eine
  erfundene waere eine Behauptung.
- 🔴 **`K-84` ist geschlossen** (**D-303**): Abschnitt 7 von `08-skill-conventions.md`
  (0.3.0) oeffnet die Zellen eines Testblatts nur noch, wenn eine Versionsaenderung eine
  **Anweisung** beruehrt. `fw-mr-description` (0.1.6) und `fw-review-support` (0.1.7)
  sind damit gehoben, **ohne dass eine Zelle altert**. ⚠️ **Preis, nicht maschinell
  pruefbar:** Die Grenze zieht ein Mensch - dieselbe Bauform wie `K-41`.
- 🔴 **`K-97` ist geschlossen** (**D-304**): Die Devin-Belege bleiben auf `SWE-1.6 Slow`
  **eingefroren**; ein Pro-Konto wird nur fuer **neue** Messgegenstaende genommen, und
  jede Matrixzeile nennt kuenftig Client **und** Modell. *Ein Modellwechsel stellt keine
  Vergleichbarkeit her - er fuegt einen dritten Gegenstand hinzu.* **Die
  Faehigkeitsmatrix misst Engines, nicht Modelle.**
- 🟢 **Der Vortrag am 2026-09-24 laeuft auf dem umbenannten Baum** (**D-302**): Die
  Folien werden nachgezogen, die aufgezeichneten Rueckfall-Belege von Buendel 3 nicht -
  **und das wird im Vortrag gesagt.** *`D-275` ist nicht falsch geworden, sein Grund ist
  weggefallen.*

### Zehn Befunde vor dem ersten Handgriff - und zwei weitere erst im Abnahmelauf

- 🔴 **Die Arbeitsflaeche war 1.529 Fundstellen in 191 Traegern, nicht 925 in 128**
  (**D-298**). `CR-2026-119` zaehlte **Backtick-Pfade**, weil ihn die Vertraeglichkeit
  mit Pruefung 12 interessierte - und sein Schlusssatz machte daraus *"die Arbeitsflaeche
  des Textlaufs"*. **436 Fundstellen in 86 lebenden Traegern stehen ausserhalb von
  Backticks**, davon 144 in `probe-pruefungen.py` als Suchtexte von Praeparationen.
  *Wer die 925 nimmt, laesst 604 stehen.* **Dieselbe Bauform wie die vier Zahlen von
  `0.87.0`.**
- 🔴 **Ein Umlaut im Beschreibungssatz einer Sonde bricht den zeilengleichen
  Vergleich nach D-49.** Beide Kodierungsumgebungen liefen mit Exit 0; der Vergleich
  fand **eine** Abweichung unter 456 Zeilen. 🟢 **Alle bestehenden Sondentexte
  schreiben ASCII-Umschrift** - das sah wie eine Schreibgewohnheit aus und ist die
  Bedingung der Abnahme. 🔴 **Die SUCHTEXTE derselben Sonde muessen die Umlaute
  dagegen tragen** - sie werden gegen die Validatormeldung gehalten. *Zwei Textsorten
  in derselben Datei, mit entgegengesetzter Regel.*
- 🔴 **Und ein Segment hat er verlaengert, das im KERN liegt - gefunden hat es
  NUR der Sondenlauf.** `templates/project-overlay` wandert nicht; der Lookbehind des
  Textlaufs erwischte es als zusammenhaengenden Pfad, **nicht als eigenes Segment einer
  Liste** (`os.path.join(KERN, "templates", "project-overlay", ...)`). **Vier Stellen,
  zwei echte Fehler: Pruefung 28 verlor stillschweigend einen von drei Traegern**, und
  ihre zweite Sonde blieb gruen. 🔴 **Der Validator meldete nichts** - Pruefung 12
  prueft Backtick-Pfade und Markdown-Links, keine Python-Ausdruecke. 🟢 **Genau das
  hat CR-2026-119 Abschnitt 3.3 vorhergesagt:** *"Der Sondenlauf ist bei dieser
  Umbenennung kein Formalakt, sondern der eigentliche Nachweis."*
- 🔴 **Der Textlauf hat selbst in fuenf regulaere Ausdruecke geschrieben** (D-299).
  `.koolie` beginnt mit einem Punkt, und der ist dort ein **Metazeichen**: Vier Muster
  wurden **zu weit**, und der Schreibschutz von `project-overlay` im Hook zugleich **zu
  eng**, weil der Schraegstrich woertlich stand, wo `[\\/]` noetig ist - *die Lehre von
  `B3`* (D-277).
- ⚠️ **Der Zeichendeckel der Wurzel-Anweisungsdatei haelt - mit 97 statt 126 Zeichen.**
  `leitwerk-core`->`.koolie/core` rechnet 11mal mit **-1**, `project-overlay`->
  `.koolie/project-overlay` 5mal mit **+8**. 🔴 **Der Durchgang vor dem Commit hat die
  Zahl gefangen: Sie war GERECHNET.** An der erzeugten Datei gemessen sind es **11.887
  von 12.000, Luft 113** - *eine Zahl, die man nicht gezaehlt hat, ist erfunden.*
- 🔴 **"Der vierte Traeger, schon wieder."** `install.py --update` fasst weder
  `.devin/config.json` noch die Laufzeitfassung des Overlays an - **45 Fundstellen in
  sieben Traegern** sind von Hand nachgezogen worden, und **genau das ist der
  Migrationshinweis** aus D-270, hier am eigenen Repositorium erprobt.
- 🔴 **Ein Chroniktraeger kann eine Zeile fuehren, die keine Chronik ist.** `CHANGELOG.md`
  nennt in Zeile 3 den Prozesspfad, `tests/protocols/README.md` in Zeile 3 den
  Testkatalog. **Beides sind Kopfzeilen mit einem AKTUELLEN Pfad**, und die
  Chronikausnahme deckte sie mit. *Der benannte Preis von Pruefung 75 - die Ausnahme
  gilt je Traeger, nicht je Zeile - ist bei ihrem ersten Lauf angefallen.*
- 🟢 **Pruefung 75 hat sich bei ihrem ersten Lauf selbst gemeldet.** Das Sondenskript
  nennt den alten Namen viermal, **weil sein Gegenstand der alte Name ist**. Daraus ist
  eine vierte, strukturell unvermeidbare Ausnahmeklasse geworden: *Eine Pruefung, die
  einen Namen sucht, muss ihn nennen - und ihre Sonde muss ihn schreiben.*
- 🔴 **`kopie()` schliesst `.git` aus**, und Pruefung 75 liest `git ls-files`. **Ohne
  `git init` haette sie in JEDER Sonde ihren dritten Ausgang genommen** (*"nicht
  messbar"*) und dabei ausgesehen wie eine, die nichts gefunden hat. Derselbe Griff wie
  bei Gegenstand 2 von Pruefung 45.
- ⚠️ **Der Pilot stand auf Kernversion `0.54.1`** - 34 Releases zurueck; das
  Uebungsrepositorium auf `0.84.0`. Beide sind gehoben und migriert.
- 🟢 **Fuenf Angaben der Uebergabe halten nachgezaehlt:** `CR-2026-122`, `D-298`,
  `K-100`, `G-21`, `UEB-32`. **Pruefung 75 war frei**, und die **Migrationsflaeche ist
  exakt die 30 Dateien mit 141 Nennungen**, die D-270 nachgezaehlt hat.
- 🔴 **Ein Nachbarfund, gemeldet und NICHT geaendert** (`K-100`): **27 Klaerungspunkte -
  `K-71` bis `K-98` - stehen in der Entscheidungstabelle** statt in der
  Klaerungstabelle, **seit `0.66.0`, 32 Releases**. Sie rendern unter den falschen
  Spaltenueberschriften. **Das ist D-264 an einer zweiten Stelle**, und Pruefung 74
  prueft nur die Faehigkeitsmatrizen.

### Migrationshinweis fuer uebernehmende Projekte

🔴 **Ein Umzug ist kein Update.** `install.py --update` schreibt an den alten Ort und
fasst die Berechtigungsdatei ueberhaupt nicht an. Die Reihenfolge:

1. **Sichern** (`git bundle --all`), **vor** dem ersten Schritt.
2. `git mv leitwerk-core .koolie/core` und `git mv project-overlay .koolie/project-overlay`
   im Zielprojekt.
3. Den Kern ersetzen: `rm -rf .koolie/core`, dann
   `git -C <framework> archive HEAD .koolie/core | tar -x -C .`
4. `python .koolie/core/install.py --update`
5. 🔴 **Die Schicht, die `install.py` NICHT anfasst, von Hand nachziehen** - gemessen am
   eigenen Repositorium: die **Berechtigungsdatei** des Packs (`_comment`, `deny`-Korb,
   **beide Hook-Kommandos**), die **Laufzeitfassung des Overlays**
   (`rules/20-project-overlay.md`), das **Overlay selbst** (`OVERLAY.md`,
   `overlay-manifest.yaml`, `documents/`, `exceptions/`, `forbidden-terms.txt`) und
   **projekteigenes** (`README.md`, `.github/`, `tools/`, **`.gitignore`**).
6. 🔴 **Die `.gitignore` ist nicht optional:** `leitwerk-core/build/out/` ist ein
   **wirksames** Ausschlussmuster, das nach dem Umzug **stumm** aufhoert zu greifen und
   die Erzeugnisse des Kerns in `git status` zeigt (D-97, Pruefung 45).
7. `python .koolie/core/tests/scripts/validate-framework.py --strict-overlay`

**Gemessen ueber beide uebernehmenden Projekte: 30 Dateien mit 141 Nennungen** (Pilot 13
mit 50, Uebungsrepositorium 17 mit 91) - dazu der Umzug von `project-overlay/`, den
D-270 noch nicht kannte.

### Bekannte Einschraenkungen

- ⚠️ **197 Traeger tragen einen geaenderten Text bei unveraenderter Version.** Gehoben
  sind vier - `08-skill-conventions.md`, beide Skills und `TEST_CATALOG.md`. Alle
  uebrigen Aenderungen sind **Namensanpassungen**, und D-303 nennt sie ausdruecklich
  nicht als Anweisungsaenderung. **Was den Stand traegt, ist nicht die Versionszeile,
  sondern Pruefung 75.**
- ⚠️ **Die Belegablagen ausserhalb des Repositoriums tragen weiter den alten Namen**
  (D-300).
- ⚠️ **`K-100` ist offen**: 27 Klaerungspunkte in der falschen Tabelle, gemeldet und
  nicht geaendert.
