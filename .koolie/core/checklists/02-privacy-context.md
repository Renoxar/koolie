...
Verhindert, dass unzulässige Inhalte (K3) oder nicht freigegebene Inhalte (K2) an den KI-Client gelangen (`.koolie/core/framework/core/02-privacy.md`, `.koolie/core/decision-trees/01-context-allowed.md`).
...
Sind K3-Inhalte bereits an den KI-Client gelangt: sofort nach `.koolie/core/framework/core/02-privacy.md` Abschnitt 5 verfahren (Sitzung beenden, Secrets rotieren lassen, Meldung an `<SECURITY_CONTACT>` und bei Personenbezug `<DATA_PROTECTION_CONTACT>`, Erfassung nach `.koolie/core/governance/INCIDENT_HANDLING.md`) – Eskalationsstufe E3 (`.koolie/core/framework/core/10-error-escalation.md`). Fehlende K2-Freigabe: Quelle nicht verwenden, gegebenenfalls Freigabe anfordern (E1).

## Ergebnis und Nachweis

Liste der verwendeten Quellen mit Klasse und – bei K2 – Freigabe (Rolle, Datum, Bereinigung) im Ergebnisbericht; Reviewer prüfen die Liste stichprobenartig (RV-Stichprobe nach `.koolie/core/framework/core/02-privacy.md` Abschnitt 4 Schritt 5).
