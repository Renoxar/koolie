...
Das Framework trennt seit D-02 zwei Formen derselben Regeln: die kanonische, **werkzeugneutrale** Langform in `.koolie/core/framework/` und die kompakte **Laufzeitform** in der Wurzel des Projekts. Die Laufzeitform ist bislang ausschließlich für einen KI-Client geschrieben.
...
| `openai-codex` | `CP-OC` | pilot | 10 von 34 | 🔴 **4 von 6** | 🔴 **Das erste Pack, dessen Belege saemtlich aus Messungen am Client stammen und nicht aus seiner Dokumentation** – es traegt keine einzige `[DOK]`-Zeile, und `FW-AK-01` ist fuer diesen Client nicht gefahren. Gemessen an einer realen Installation sind `B2`, `B4`, `B6`, `H1` bis `H3`, `R1`, `R5`, `S1` und `S5`; **drei Zeilen sagen `BELEG OFFEN`** (`S2`, `S3`, `M3`), dazu `X2` dauerhaft. 🔴 **ZWEI KERNZUSAGEN SIND `[NICHT ABBILDBAR]`** – `B3` und `B5` –, und damit greift Abschnitt 4 dieser Datei vollstaendig: **keine Inbetriebnahme ohne Freigabe durch `<SECURITY_CONTACT>`** |
...
| 0.7.0 | 2026-09-23 | 🟢 **Das dritte Client Pack ist da, und mit ihm die zweite Ausgabeform der Berechtigungsdatei** (`CR-2026-133`, D-346 bis D-349). Die Regelmenge des Kerns zerfaellt bei `openai-codex` in **zwei** Erzeugnisse: Pfade in einer TOML-Tabelle, Befehle in einer eigenen Regelsprache. 🔴 **Sechs Pruefungen lesen die Form `json` und erreichen dieses Pack deshalb nicht** – sie stehen seit `1.4.0` in `FORMATGEBUNDENE_PRUEFUNGEN`, und **Pruefung 87** verlangt ihre Nennung in Abschnitt 5 des Packs. *Eine Luecke, die erklaert ist, ist eine Aussage; eine, die nur besteht, ist ein blinder Fleck.* 🔴 **Und der Schutz-Hook hat bei diesem Client zunaechst nichts gesperrt:** Seine Sperrform war clientgebunden, ohne dass es irgendwo stand (**Pruefung 86**, D-347) | `<FRAMEWORK_OWNER>` |
