...
Dieses Verzeichnis enthält alles, was Devin Local aus dem Repository liest. Es ist die einzige Devin-spezifische Schicht des Frameworks (Tool Independence). Alle Inhalte sind Konkretisierungen der werkzeugneutralen Regeln in `.koolie/core/framework/`.
...
| `mcp_config.json.example` | Vorlage für MCP-Server (Standard: keine) | `[DOK]` Dateiname; **Struktur am Client erhoben** (2026-09-16): Container `mcpServers`, je Eintrag `command`, `args` und `transport`. **Der Standardablageort des Clients ist der unversionierte** `mcp_config.local.json` – wer einen Server ohne Angabe des Geltungsbereichs hinzufügt, schreibt nicht in die versionierte Datei |
...
Die Regelablage enthält die clientspezifische Laufzeitfassung der Framework-Regeln. Die kanonische, werkzeugneutrale Langform liegt in `.koolie/core/framework/`. Bei Widersprüchen gilt die Langform; die Laufzeitfassung wird dann korrigiert.
...
Die Nummern bilden die Ladereihenfolge im Dateisystem ab, nicht die Priorität. Die Priorität regelt `AGENTS.md` Abschnitt 2 beziehungsweise `.koolie/core/governance/PRIORITY_HIERARCHY.md`.
...
`python3 .koolie/core/tests/scripts/validate-framework.py` prüft dieses Verzeichnis (JSON-Gültigkeit, Kernregeln in `config.json`, Frontmatter der Regeln und Skills, Zeichenlimits, verbotene Inhalte).
