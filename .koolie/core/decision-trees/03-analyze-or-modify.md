...
2. **Plan gesucht?** Soll ein Vorgehen oder Änderungsplan entstehen, aber noch nichts umgesetzt werden? → **M2 Guided Planning** (`fw-plan`, `fw-bugfix-prepare`). M2 ist Pflichtstation vor M3 ab Kontrollstufe mittel; M4 und M5 folgen `09-risk-model.md` Abschnitt 3.
...
flowchart TD
    ...
    B -- "ja" --> M1["M1 Read-only Analysis<br/>fw-repo-analyze, fw-code-explain,<br/>fw-change-analyze, fw-error-analyze,<br/>fw-review-support"]
    B -- "nein" --> C{"Soll zunächst nur ein<br/>Plan entstehen?"}
    C -- "ja" --> M2["M2 Guided Planning<br/>fw-plan, fw-bugfix-prepare<br/>Pflicht vor M3 ab Stufe mittel"]
