...
## Stand nach Release 1.12.0 (2026-09-26)
...
### Geplant: Die Regelablage von `devin-desktop` und die Pfadeinträge von `openai-codex` – Ziel-Release **1.12.1**

Befunde des Messtages von `1.12.0` (D-410). **`K-156`:** Mit `read_config_from.windsurf: false` – der Einstellung aus Zeile `R6`, die die Anweisungsdatei im Benutzerprofil fernhält (D-290) – lädt Devin CLI 3000.11.1 die Regelablage `.devin/rules/` nicht; das Modell sieht von der Regelschicht nur `AGENTS.md`. **`K-157`:** `openai-codex` 0.157.0 meldet, dass es die `:workspace`-Pfadeinträge des Rechteprofils nicht erkennt und ignoriert. Beide brauchen eine Abhilfe, eine Messung an einer Installation und das Nachziehen der Einstufungen – also Kontingent.

### Erledigt mit `1.12.0`: Der Mehrprojektfall und die Token-Last

> 🟢 **Gefahren mit `1.12.0`** (`CR-2026-148`, D-407 bis D-410). `K-154`: der Messapparat findet den Kern unter `.koolie/core`, das Füllskript entfaltet jeden Platzhalter (D-407); `K-138`: je Pack gemessen, Startort-Bedingung in den Matrizen, Overlay-Vorlage und Übernahmeleitfaden Abschnitt 4 (D-408); `K-144` (1) und (3): gemessen, Kostenabschnitt im Übernahmeleitfaden Abschnitt 7 und in Kapitel 1 (D-409). Die Senkung (`K-144` (2)) und die Meldung eines falschen Startorts (`K-159`) bleiben offen.
...
| ~~`1.9.3`~~ ✅ | `framework/skills/` und die Skills der Role Packs, `templates/`, `clients/_template/` – D-393 |

**Die Regeln der Durchsicht** stehen in D-381: kein Regelinhalt geändert, ein Widerspruch
wird ein Klärungspunkt; eine Laufzeitdatei wird nicht länger. **In einer `SKILL.md` ändert die
Durchsicht keine Anweisung** (D-393): nur Schreibung, Verweise auf dieselbe Sache, Erläuterung und
Form; ein geänderter Skill wird um PATCH gehoben, sein Änderungsverlauf nennt die Art der Änderung,
und keine Ergebniszelle altert (D-303).
Die Befunde der Durchsicht von `1.9.2` (`K-139` bis `K-143`) und `1.9.3` (`K-146`, `K-148` bis `K-150`)
werden mit `1.11.0` entschieden, `K-151` mit `1.10.0`.

> 🟢 **Gefahren mit `1.9.3`** (`CR-2026-145`, D-393, D-394). Fünf parallele Durchsichten über die Skills, die
