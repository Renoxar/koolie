...
### Erledigt mit `1.12.1`: Die Regelablage von `devin-desktop` und die Pfadeinträge von `openai-codex`

> 🟢 **Gefahren mit `1.12.1`** (`CR-2026-149`, D-411 bis D-413). `K-156`: `windsurf: true`, weil Devin CLI die eigene Regelablage sonst nicht lädt (3000.11.1 und 3000.11.3, gegen die Dokumentation); `copilot`, `opencode`, `zed` auf `false`; `install.py` meldet belegte Windsurf-Kanäle (D-411). `K-157`: die Tabelle `:workspace_roots`, die Pfadseite von `B4` gemessen (D-412). Neu ohne Ziel-Release: `K-160` (`deny`-Globs bei Codex) und `K-161` (`glob`-Regeln bei Devin) (D-413).
...
> Ausprobieren, englische Fassungen von README und Quickstart, Sprachstrategie im Dokumentationsstandard
> (Abschnitt 5), Einstiegsdokumente der Wurzel als Klasse A. Der Rest des Auftrags bleibt bei `K-166`.
...
> 🟢 **Gefahren mit `1.14.1`** (`CR-2026-152`, D-425 bis D-432). Zwei Skills angeglichen, drei Zellen von `fw-plan`
> und `SK-007-P01` gepflegt, der Messbaum ohne Aufzeichnungen, die Formregel für Überschriften; 18 Zellen
> nachgemessen: 17 tragen, `SK-005-P01` bleibt offen (→ `1.14.2`).
...
Idee und Auftrag des Owners vom 2026-09-26 (`K-155`, `CR-2026-147` E9), mit D-422 ans Ende des Plans gestellt; seit D-436 folgt ihm `1.18.0`. Gitea bleibt führend; das öffentliche
