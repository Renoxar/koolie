...
## Stand nach Release 0.89.0 (2026-09-22)
...
```
FEHLER   .koolie/core/docs/ROADMAP.md: Kriterium 4 von D-11 (Decision Records ohne
...
| Womit wurden die Fundstellen ersetzt? | **Mit dem Begriff, nicht mit einem Platzhalter.** **Keiner der vierzehn Träger wird gerendert** – nachgelesen in `render_for_client` und den `shared_core`/`shared_seed`-Einträgen beider Manifeste. Ein `<SKILLS_DIR>` in `.koolie/core/prompts/` bliebe für immer stehen, und `prompts/README.md` erklärt spitze Klammern als Overlay-Werte: **eine Marke mit zwei Bedeutungen** |
...
- **`K-50`:** Ob die Umbenennung einen **maschinellen** Migrationspfad braucht oder ein
  Migrationshinweis genügt. `install.py --update` schreibt `.koolie/core/` nicht; ein
...
| *Nicht gesucht:* Was lag daneben? | **Sechs versionierte `.pyc`-Dateien** unter `.koolie/core/`. Zwei davon standen beim Auschecken als geändert da, ohne dass jemand etwas getan hätte |
...
| *Nicht gesucht:* Was lag unter dem verlorenen Anker? | **Ein echter Fall von B07.** Das Übungs-Overlay nannte seine Pfadplatzhalter ohne spitze Klammern; Prüfung 28 fand ihre Deklaration nicht und **bestand still**. Darunter: `AGENTS.md`, `.devin/` und `.koolie/project-overlay/` als „feste Ausschlüsse" – schreibgesperrt, aber lesbar (D-55) |
...
| Name | Das Framework heißt **Koolie**, das Kernverzeichnis `.koolie/core/`; der Name folgt damit dem Inhalt | D-19, `CR-2026-009` |
...
`.koolie/core/VERSION` und jedes Versionsfeld gegen `MAJOR.MINOR.PATCH`. Seit D-25 nennen nur
...
- **Jeden Pfad des Auslieferbestands:** `.koolie/core/` wird `koolie-core/`. Der Wert des
...
- **Die Verzeichnisse beider übernehmender Projekte.** `install.py --update` schreibt
  `.koolie/core/` **nicht** – das Verzeichnis wird von Hand ersetzt (Abschnitt *Ein
