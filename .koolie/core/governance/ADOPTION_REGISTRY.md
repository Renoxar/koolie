...
|---|---|---|---|---|---|
| `devpacks/otp-generator` | **Pilot** – ein echtes Projekt, keine Spielwiese | `claude-code` | **1.8.0** | `0.3.13` | 2026-09-25 |
| `devpacks/test-devin-framework` | **Übungsrepositorium** – Meßgegenstand der Sitzungstests, 31 Präparationen | `devin-desktop` | **1.8.0** | `1.4.7` | 2026-09-25 |

🟢 **STAND 2026-09-25: BEIDE PROJEKTE TRAGEN `1.8.0`, UND DIE HEBUNG IST DORT COMMITTET.** 🆕 **Beide mit Lieferumfang `voll`** (`.koolie/core/LIEFERUMFANG`, D-367) – das Heben mit `1.8.0` hat die Datei zum ersten Mal geschrieben; reduziert ist keines. **`1.7.0` war das erste Release, mit dem beide Projekte über das Werkzeug gehoben wurden** – `install.py --target <projekt> --update` aus dem Arbeitsbaum des Frameworks (D-362) statt der Handgriffe `rm -rf` und `tar`; die Starter selbst erreichen kein Projekt, sie liegen in der Wurzel des Frameworks. `1.6.0` bringt sechs Musterdokumente allgemeiner Praktiken in das Overlay-Muster `general` (D-359 bis D-361) – **sie erreichen keines der beiden Projekte**, beide führen eigene Dokumente derselben Typen. 🔴 **Die erweiterte Prüfung 8 hält jetzt jeden Manifesteintrag gegen die Ablage:** 14 Einträge im Pilot, 7 im Übungsrepositorium, dazu drei Regeldateien – alle vorhanden, beide still. `1.5.0` bringt das Overlay-Muster *General Development* und Prüfung 89 (D-355 bis D-358); **das Muster erreicht keines der beiden Projekte** – es wirkt nur bei der Erstinstallation. 🔴 **Prüfung 89 hat im Übungsrepositorium vier Befunde geliefert:** Seine Laufzeitfassung **ersetzte** die Werte von `<ALLOWED_PATHS>`, `<TEST_PATHS>`, `<DOC_PATHS>` und `<READ_ONLY_PATHS>`, statt die Platzhalter zu binden (D-160); nachgezogen mit der Hebung. Der Pilot band alle vier und bestand. `1.4.4` gibt dem Kopierweg des Kerns einen Warnsatz und `install.py` eine Auskunft über ein mitkopiertes Kennzeichen des Quellrepositoriums (D-354); **beide Projekte wurden, wie seit D-333, nur mit `.koolie/core/` gehoben – die Auskunft schweigt dort.** `1.4.3` ordnet ein und ändert nichts, was ein Projekt erreicht (D-353); **die beiden Berechtigungsdateien dieser Liste sind zugleich sein Meßgegenstand** – 37 Projekteinträge, die ein Erzeugen bei `--update` überschrieben hätte. `1.4.2` berichtigt die Auskunft über ignorierte Kerndateien (D-352); im Pilotprojekt ist sie der erste Lauf der berichtigten Fassung gegen die projekteigene Zeile `build/`. Die Hebung auf `1.4.0` war auf Entscheidung des Framework Owners verschoben; sie ist mit `1.4.1` nachgeholt, in einem Schritt über beide Releases. ⚠️ **Die Spalte `Overlay-Version` stand bis dahin auf `0.3.3` und `1.1.0`, während die Projekte seit `1.3.0` `0.3.5` und `1.3.0` trugen** – Prüfung 82 hält nur die Spalte `Framework-Version` (`CR-2026-134`). *Eine Spalte, die niemand zählt, ist eine, die niemand veralten sieht.*
