...
1. M1 Read-only Analysis (nur lesen, Befunde mit Fundstellen); M2 Guided Planning (nur Plan-Datei außerhalb des Quellcodes); M3 Controlled Modification (Ändern im freigegebenen Scope, freigegebene Befehle); M4 Test and Validation (nur Testpfade, kein Produktivcode); M5 Documentation Support (nur Doku-Pfade, nur lesende Git-Befehle). Quelle: `.koolie/core/framework/core/05-working-model.md`.
...
5. Der KI-Client darf: Optionen analysieren (Name, Version, Lizenzangabe, Alternativen) und nach menschlicher Einführung der Bibliothek die Verwendungen gemäß bestätigtem Plan anpassen. Der KI-Client darf nicht: die Abhängigkeit einführen, Manifest/Lockfile ändern (V3, R9 → hoch, `.koolie/core/checklists/07-new-dependency.md`).
