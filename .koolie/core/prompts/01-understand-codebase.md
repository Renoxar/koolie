...
- K3 gemäß `.koolie/core/framework/core/02-privacy.md`: Secrets, `.env`-Werte, personenbezogene Echtdaten, Produktionsdaten, interne Adressen.
...
- K2-Inhalte ohne dokumentierte Freigabe nach `.koolie/core/framework/core/02-privacy.md` Abschnitt 4.
...
| `{faktor}` | MUSS | K1 | Auslösender Risikofaktor R1–R13 gemäß `.koolie/core/framework/core/09-risk-model.md` |
...
Betriebsmodus: M1 Read-only Analysis (.koolie/core/framework/core/05-working-model.md). Du erzeugst, änderst, verschiebst oder löschst keine Datei und führst keine Befehle aus.
...
- Ergebnisbericht nach `.koolie/core/framework/core/05-working-model.md` Abschnitt 3.6 (keine Änderungen, keine Befehle).

## 7. Prüfschritte

- [ ] Mindestens drei Fundstellen geöffnet und den jeweiligen Befund bestätigt (`.koolie/core/framework/core/00-principles.md`, P4).
...
- [ ] Keine zitierten Konfigurationswerte, Secrets oder personenbezogenen Daten im Bericht (`.koolie/core/checklists/02-privacy-context.md`).
- [ ] Ausgeschlossene Pfade wurden nicht gelesen; der Bericht benennt sie als ausgeschlossen.
- [ ] Offene Fragen an das Team mit der Mentorin oder dem Mentor besprochen; Auffälligkeiten nicht ungeprüft weitergegeben (`.koolie/core/checklists/09-onboarding.md`).
