...
- K3 gemäß `.koolie/core/framework/core/02-privacy.md`; `<EXCLUDED_PATHS>`; Werte aus Konfigurations- und Umgebungsdateien; Produktions- und Infrastrukturdetails.
- Ticket-Kommentare, Anhänge und Kundenkommunikation.
- Externe Quellen (Websuche, Paketregister) zur Optionsfindung – neue Abhängigkeiten sind Entscheidungen des Menschen (V3, `.koolie/core/checklists/07-new-dependency.md`).
...
```text
Ziel: Änderungsplan exakt nach .koolie/core/templates/PLAN_TEMPLATE.md für die Aufgabe unten – alle zehn Abschnitte, Bestätigungsstatus „entwurf". Keine Umsetzung.
Betriebsmodus: M2 Guided Planning (.koolie/core/framework/core/05-working-model.md). Du änderst keine Datei im Repository (auch keine Plan-Datei im Repository und keine „vorbereitenden" Änderungen) und führst keine Befehle aus.
...
- [ ] Optionswahl selbst getroffen und im Plan vermerkt; Optionen mit neuer Abhängigkeit nach `.koolie/core/checklists/07-new-dependency.md` geprüft (V3).
