...
# <wurzel>/.koolie/core/tests/erhebungen/ - also VIER Ebenen unter der Wurzel.
#
# 0.88.0: Bis dahin waren es drei, weil der Kern ein einziges Verzeichnissegment
# war (`leitwerk-core`). Der Umzug nach `.koolie/core` haette WURZEL auf `.koolie/`
# gesetzt, und jeder abgeleitete Kernpfad haette ein Segment zu wenig getragen.
# Die Zahl wird deshalb aus CORE_TIEFE gerechnet und nicht im Quelltext gezaehlt
# (D-299).
CORE_REL = ".koolie/core"
CORE_TIEFE = CORE_REL.count("/") + 1

_HIER = os.path.dirname(os.path.abspath(__file__))
_KERN = os.path.dirname(os.path.dirname(_HIER))      # die Lage aus CORE_REL
WURZEL = _KERN
...
def _innerhalb(pfad, wurzel):
    ...
    Ein Praefixvergleich auf der Zeichenkette nimmt ein GESCHWISTERverzeichnis,
    dessen Name mit dem der Wurzel beginnt, faelschlich als deren Kind:
    `...\\koolie-erhebungen` neben `...\\koolie`. Das ist genau die Bauform von
    D-219, eine Ebene tiefer: ein Praefix, das mehr erfasst als sein Gegenstand.

    0.88.0: Der Satz nannte bis dahin `...\\leitwerk-erhebungen` neben
    `...\\leitwerk`. Die Belegablagen behalten ihren alten Namen (sie sind
    datierte Aufzeichnungen und werden nicht umbenannt), das Repositorium nicht -
    damit ist GENAU DIESES Paar kein Beispiel mehr. Der Fall bleibt herstellbar,
    sobald eine Ablage neben dem Repositorium liegt und dessen Namen als Praefix
    traegt; deshalb steht hier ein Beispiel mit dem neuen Namen und nicht der
    alte Satz.
    """
    try:
...
def kernversion():
    ...
    """
    p = os.path.join(WURZEL, ".koolie/core", "VERSION")
...
def blaetter(wurzel=None):
    ...
    rahmen = os.path.join(wurzel, ".koolie/core", "framework")
