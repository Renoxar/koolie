...
# C:\\lw-b3 -> C:\\lw-b4.
"""Nimmt den Zustand der Messbaeume unter C:\\lw-b4 auf und vergleicht zwei Aufnahmen.

    python zustand-b4.py vorher
    python zustand-b4.py nachher

🔴 ANDERS ALS IN BUENDEL 1 UND 2 ist eine Veraenderung hier der MESSWERT, nicht
der Befund: Alle drei Skills dieses Buendels schreiben. Die Aufnahme sagt je
Baum, WAS ein Lauf geaendert hat - und damit, ob ein Negativfall wirklich nichts
angefasst hat. Ein Bericht ist keine Aufzeichnung.

`frontend/node_modules` bleibt aussen vor - es ist in jedem Baum dieselbe
Verzeichnisverbindung. Dass kein Lauf dorthin geschrieben hat, belegt
`node-waechter.py` ueber Dateizahl und Groesse des gemeinsamen Bestands.
"""
import hashlib
import io
import json
...
TEILE = [os.path.join("frontend", "src"), os.path.join("backend", "src"),
         "api-contracts", "docs", ".claude", ".koolie/core", ".github",
         "CLAUDE.md", "README.md"]
