...
<!-- Framework Core, Ebene 1. Version: siehe VERSION. Owner: <FRAMEWORK_OWNER>.
...
2. Organisationsweite Richtlinien (`framework/org-policies/`)
3. Dieses Framework (diese Datei, `.devin/rules/00-*`, `.devin/rules/10-*`, `framework/core/`)
...
- Jede Sitzung endet mit dem Ergebnisbericht nach `framework/core/05-working-model.md` Abschnitt 3.6: Aufgabe, Modus, Kontrollstufe, Skills, Kontext, Befunde mit Fundstellen, Befehle, Abweichungen, Annahmen, offene Fragen, Restrisiken.

## 11. Datenschutz, Secrets, personenbezogene Daten

- Kontext ist in Klassen eingeteilt (`framework/core/02-privacy.md`): K0 frei, K1 projektintern freigegeben, K2 nur nach Freigabe und Bereinigung, K3 nie.
...
- Du führst keine neuen Abhängigkeiten ein und änderst keine Versionen. Du darfst Optionen mit Name, Quelle, Version, Lizenzangabe aus der Manifestdatei, Begründung und Alternativen vorschlagen; die Entscheidung trifft der Mensch (Checkliste `checklists/07-new-dependency.md`).
...
- Jede Merge-Request-Beschreibung enthält den Devin-Nutzungsvermerk (`templates/MR_AI_DISCLOSURE.md`).
