...
Stand Release 0.1.0: 155 Markdown-Dateien plus Konfigurations- und Skriptdateien. Übersicht je Bereich (Dateien einschließlich Vorlagen und READMEs):

| Bereich | Dateien | Kerninhalte |
|---|---|---|
| Wurzel | 8 | `AGENTS.md`, `AGENTS.local.md.example`, `README.md`, `VERSION`, `CHANGELOG.md`, `OWNERS.md`, `.gitignore` |
| `.devin/` | 61 | 7 Regeldateien (+2 Vorlagen), 12 Skills × 4 Dateien, Subagentenprofil, `config.json`, `hooks.v1.json`, MCP-Vorlage, 2 READMEs |
...
| `project-overlay/` | 17 | Overlay-Vorlage, Manifest, 13 Dokumenttyp-Verzeichnisse, Ausnahmeregister, README |
...
| `tests/` | 6 | Testkatalog, Sperrbegriffsliste, 4 Skripte |
...
{{EMBED-RAW:docs/PLACEHOLDER_REGISTRY.md:2}}
...
| V3 | Hook-Eingabeschema (Feldnamen wie `tool_name`/`tool_input`) und Blockierverhalten; anschließend Umstellung des Schutz-Hooks auf fail-closed | `tests/scripts/hook-check-secrets.py`, `hook-overlay-status.py`, `.devin/hooks.v1.json` |
...
Im Repository unter `examples/`: ausgefüllte Overlay-Laufzeitfassung, vollständiger Devin-Ergebnisbericht und Merge-Request-Beschreibung mit Nutzungsvermerk – alle ausdrücklich synthetisch und mit erfundenen Bezeichnern (siehe `examples/README.md`).
