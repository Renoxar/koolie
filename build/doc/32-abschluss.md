...
Vor der finalen Ausgabe wurde die geforderte Qualitätssicherung durchgeführt – automatisiert (Strukturvalidator über alle 155 Markdown-Dateien: 0 Fehler, 0 Warnungen; Inhaltsprüfungen auf Secret-Muster, E-Mail-Adressen, IP-Adressen, interne Hostnamen, nicht gelistete URLs; Mermaid-Syntaxprüfung aller Diagramme: valide) und im Durchsichtsverfahren. Ergebnis je Prüffrage:
...
| 3 | Unbelegte Devin-Funktionen als Tatsache behauptet? | Nein. Jede Produktaussage trägt `[DOK]` mit Quellenzuordnung (Anhang 31.3), `[EMPF]` oder `[KONZ]`; Offenes ist als `<VERIFY AGAINST CURRENT DEVIN DOCUMENTATION>` markiert und in Anhang 31.4 konsolidiert. Einschränkung transparent: `[DOK]` heißt dokumentationsbelegt, nicht installationsgeprüft (AP2). |
| 4 | Framework Core und Project Overlay sauber getrennt? | Ja. Platzhalter-Schnittstellen, getrennte Ownership, technische Schreibsperren, Integritätsblock in `config.json`, strikte Overlay-Validierung; Grenzfälle über Entscheidungsbaum 6 (Kap. 8). |
...
| 8 | Rückfragen bei relevanten Unklarheiten verlangt? | Ja. No Assumption Policy in AGENTS.md, jedem Skill (Rückfragenregeln, [RÜCKFRAGE]-Punkte), jeder Prompt-Vorlage und im Arbeitsablauf Schritt 4; Testklasse FI prüft es. |
...
| 10 | Menschliche Prüfung an allen relevanten Stellen verankert? | Ja. P1/P5, Delegationsverbote V1/V2, Freigabepunkt Schritt 9, Review-Regeln mit Stufen-Tiefe, MR-Checkliste, „keine Stufe ersetzt menschliche Prüfung" (FW-CORE-09), Devin erklärt nie etwas für freigegeben. |
...
2. **Nutzungsumfang (K-04):** Nur Devin Desktop lokal (Framework-Standard) oder Freigabe von Cloud-Sessions/CLI mit erweitertem Sicherheitsmodell.
...
6. **Zielversion von Devin Desktop** für AP2 und den Pilot.
...
1. **A-01:** Das Projekt-Repository kann im Wurzelverzeichnis um `AGENTS.md` und `.devin/` ergänzt werden (sonst eigener Workspace-Zuschnitt nötig).
...
4. **A-04:** Verarbeitung der Devin-Anfragen erfolgt über Anbieter-/Modellanbieter-Infrastruktur (kein lokales Modell) – Grundlage der restriktiven Kontextklassen.
5. **A-05:** Die in der CLI-Dokumentation beschriebenen Mechanismen gelten für Devin Local in Devin Desktop (die Desktop-Dokumentation verweist auf geteilte Modi/Mechanismen); Absicherung über die VERIFY-Liste und AP2.

## Verifikationsbedarf

Gegen die aktuelle offizielle Devin-Dokumentation beziehungsweise in einer Zielinstallation zu prüfen: die zehn konsolidierten Punkte V1–V10 aus Anhang 31.4 – Zeichenlimits der Regeln unter Devin Local; exakte `config.json`-Schemadetails; Hook-Eingabeschema (danach fail-closed als Standard); Skill-Discovery `.agents/skills/` und `@skills:`-Verhalten; Frontmatter-Toleranz; MCP-Dateistruktur; Codebasis-Indexierung; Spaces-Kontextreichweite; Wirkung additiver Skill-Permissions; reales Ladeverhalten der always-on-Summe. Prüfweg: Roadmap-AP2 mit Protokollpflicht; laufend: Testklasse AK im Release-Zyklus.
...
2. **Devin-Funktionalitäten validieren (AP2):** Testinstallation mit fixierter Zielversion; die zehn VERIFY-Punkte protokolliert abarbeiten; Hook auf fail-closed stellen; Belegstatus-Tabellen aktualisieren.
...
4. **Erstprojekt konfigurieren (AP4):** Overlay vollständig ausfüllen, `config.json` befüllen, erstes Technology Pack für `<TECH_STACK>` erstellen, Übungsrepository erzeugen; strikte Validierung.
