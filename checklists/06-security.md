...
Operationalisiert das Sicherheitsmodell (`framework/core/03-security.md`) für Änderungen mit Sicherheitsbezug und für die Prüfung KI-typischer Schwächen (T2, T4, T5, T6).
...
- [ ] **MUSS** Abhängigkeiten unverändert; vorgeschlagene neue Abhängigkeiten laufen über `checklists/07-new-dependency.md` (T4); Paketnamen exakt gegen die Quelle geprüft (halluzinierte Namen, Typosquatting).
...
Fund von Secrets oder personenbezogenen Echtdaten: S3/E3 (`framework/core/10-error-escalation.md`, `framework/core/02-privacy.md` Abschnitt 5). Verdacht auf ausgenutzte Schwachstelle oder Datenabfluss: V9 – keine eigenständige Weiterarbeit, Meldeweg der Organisation. Unaufgelöste MUSS-Punkte blockieren den Merge.
