...
Stellt sicher, dass ein neues Projekt das Framework vollständig, unverändert im Core und mit ausgefülltem Overlay übernimmt (`docs/ADOPTION_GUIDE.md`; Grundsatz: Ein Projektwechsel betrifft nur Ebene 4).
...
- [ ] **MUSS** Ergebnis der Datenschutz- und Vertragsprüfung liegt vor und ist im Overlay referenziert (K-06; ohne Ergebnis bleibt die restriktivste Auslegung nach `framework/core/02-privacy.md` Abschnitt 1.3).
- [ ] **MUSS** Planstufe und administrativ erzwungene Team-Einstellungen sind dokumentiert (`framework/org-policies/`, K-05); Training-Opt-out beziehungsweise vertragliche Regelung nachgewiesen (`<TBD: Nachweis der Einstellung>`).
- [ ] **SOLL** Abbildung des Klassifizierungsschemas der Organisation auf K0–K3 liegt vor (`framework/org-policies/MAPPING_CLASSIFICATION.md`).

### Technische Integration

- [ ] **MUSS** Framework-Release in das Projekt-Repository integriert (`AGENTS.md`, `.devin/`, `framework/`, `templates/`, `checklists/`, `decision-trees/`, `prompts/`, `onboarding/`, `tests/`); Framework-Version im Overlay notiert.
...
- [ ] **MUSS** `tests/forbidden-terms.txt` projektlokal mit den realen Namen des Projekts befüllt (Datei verbleibt projektlokal).
- [ ] **MUSS** `python3 tests/scripts/validate-framework.py --strict-overlay` läuft ohne Fehler.
...
- [ ] **MUSS** Basistests des Testkatalogs auf dem Übungsrepository ausgeführt (mindestens die Klassen PI, DS, SC aus `tests/TEST_CATALOG.md`); Ergebnisse dokumentiert.
- [ ] **MUSS** Onboarding für die ersten Nutzerinnen und Nutzer geplant (`checklists/09-onboarding.md`); bis zur Freigabe arbeiten alle begleitet.
- [ ] **SOLL** Pilotparameter festgelegt (`pilot/PILOT_CONCEPT.md`: Referenzbasis, Pilotgruppe, `<PILOT_DURATION>`, Anwendungsfälle, Review-Punkte).
