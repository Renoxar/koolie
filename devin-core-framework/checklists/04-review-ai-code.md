...
Operationalisiert die Prüfpunkte RV1–RV12 aus `devin-core-framework/framework/core/07-review-rules.md` für die tägliche Anwendung. Ein Devin-Befund (`fw-review-support`) ersetzt keine dieser Prüfungen.
...
- [ ] **MUSS** (RV7) Eingabevalidierung und Autorisierungsprüfung in neuen Pfaden; Fehlermeldungen ohne Interna; kein Logging sensibler Daten; keine hartcodierten Geheimnisse; bei Treffern `devin-core-framework/checklists/06-security.md` anwenden.
- [ ] **MUSS** (RV8) Keine neue oder erweiterte Verarbeitung personenbezogener Daten; sonst Stufe hoch und `<DATA_PROTECTION_CONTACT>`.
- [ ] **MUSS** (RV6) Abhängigkeiten, Versionen, Lockfiles, Paketquellen unverändert; sonst `devin-core-framework/checklists/07-new-dependency.md` und regulärer Prozess.
...
- [ ] **SOLL** Review-Tiefe entspricht der Kontrollstufe (`devin-core-framework/framework/core/07-review-rules.md` Abschnitt 3); bei Stufe mittel führt die Reviewerin oder der Reviewer die Tests selbst aus.

## Abbruch- und Eskalationskriterien

Befunde, die auf Scope-Verlassen, erfundene Fundstellen oder umgangene Quality Gates hindeuten, stoppen die Übernahme (E0/E1). Systematische Befunde über mehrere Devin-Änderungen werden an den Framework Owner gemeldet (`devin-core-framework/governance/FEEDBACK_PROCESS.md`, E4). Sicherheits- oder Datenschutzbefunde: Prozess der Organisation, Erfassung nach `devin-core-framework/governance/INCIDENT_HANDLING.md` (E3).
