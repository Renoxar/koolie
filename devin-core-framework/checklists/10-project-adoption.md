...
Stellt sicher, dass ein neues Projekt das Framework vollständig, unverändert im Core und mit ausgefülltem Overlay übernimmt (`devin-core-framework/docs/ADOPTION_GUIDE.md`; Grundsatz: Ein Projektwechsel betrifft nur Ebene 4).
...
- [ ] **MUSS** Ergebnis der Datenschutz- und Vertragsprüfung liegt vor und ist im Overlay referenziert (K-06; ohne Ergebnis bleibt die restriktivste Auslegung nach `devin-core-framework/framework/core/02-privacy.md` Abschnitt 1.3).
- [ ] **MUSS** Planstufe und administrativ erzwungene Team-Einstellungen sind dokumentiert (`devin-core-framework/framework/org-policies/`, K-05); Training-Opt-out beziehungsweise vertragliche Regelung nachgewiesen (`<TBD: Nachweis der Einstellung>`).
- [ ] **SOLL** Abbildung des Klassifizierungsschemas der Organisation auf K0–K3 liegt vor (`devin-core-framework/framework/org-policies/MAPPING_CLASSIFICATION.md`).
...
- [ ] **MUSS** `python3 devin-core-framework/tests/scripts/validate-framework.py --strict-overlay` läuft ohne Fehler.
...
- [ ] **MUSS** Basistests des Testkatalogs auf dem Übungsrepository ausgeführt (mindestens die Klassen PI, DS, SC aus `devin-core-framework/tests/TEST_CATALOG.md`); Ergebnisse dokumentiert.
- [ ] **MUSS** Onboarding für die ersten Nutzerinnen und Nutzer geplant (`devin-core-framework/checklists/09-onboarding.md`); bis zur Freigabe arbeiten alle begleitet.
- [ ] **SOLL** Pilotparameter festgelegt (`devin-core-framework/pilot/PILOT_CONCEPT.md`: Referenzbasis, Pilotgruppe, `<PILOT_DURATION>`, Anwendungsfälle, Review-Punkte).
