<!-- Laufzeitregel des Client Packs claude-code. Inhaltlich identisch zur Fassung
     anderer Client Packs; abweichend ist nur der Lademechanismus.
     Zweck: Projektspezifische Laufzeitfassung des Project Overlays (Ebene 4). Immer aktiv. Wird vom Projekt aus project-overlay/OVERLAY.md gepflegt; darf keine Framework-Regeln lockern.
     Ladeverhalten: Wird über den Import in CLAUDE.md immer geladen.
     Entsprechung bei Clients mit Ladetriggern: trigger: always_on. -->
...
- Overlay-Version: `<TBD: Version>` · Framework-Version: siehe `devin-core-framework/VERSION`
- Overlay Owner (Rolle): `<APPROVAL_ROLE>`

## Arbeitsbereich

- Erlaubte Pfade (`<ALLOWED_PATHS>`): `<TBD: Liste, zum Beispiel src/**, test/**, docs/**>`
- Ausgeschlossene Pfade (`<EXCLUDED_PATHS>`): `<TBD: Liste, zum Beispiel deploy/**, infra/**, config/prod/**>` – zusätzlich immer: Secret-Dateien, `.claude/`, `CLAUDE.md`, `project-overlay/`
