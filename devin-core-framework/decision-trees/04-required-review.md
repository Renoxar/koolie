...
| Quelle | `devin-core-framework/framework/core/07-review-rules.md`, `09-risk-model.md`, `devin-core-framework/checklists/04-review-ai-code.md` |
...
3. **Änderungen (M3/M4/M5) nach Kontrollstufe:**
   - **niedrig:** Selbstreview anhand `devin-core-framework/checklists/04-review-ai-code.md` (vollständiges Lesen des Diffs; mindestens RV1, RV2, RV5, RV9, RV10) plus reguläres Projekt-Review und alle Quality Gates.
...
4. **Sonderauslöser unabhängig von der Stufe:** Berührung von Abhängigkeiten → `devin-core-framework/checklists/07-new-dependency.md`; Sicherheitsmuster im Diff → `devin-core-framework/checklists/06-security.md`; neue Verarbeitung personenbezogener Daten → `<DATA_PROTECTION_CONTACT>`; gemeldete Injektionsversuche → `<SECURITY_CONTACT>`.
5. **Abschluss:** Übernahme ausschließlich über den bestehenden Review- und Freigabeprozess (`devin-core-framework/checklists/08-merge-request.md`); Devin-Nutzungsvermerk ist Pflicht.
