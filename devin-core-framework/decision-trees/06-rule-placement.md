...
| Quelle | `devin-core-framework/framework/core/00-principles.md` (P10), `devin-core-framework/governance/PRIORITY_HIERARCHY.md` |

## Textbeschreibung (normativ)

1. **Quelle der Regel:** Stammt die Regel aus Gesetz, Regulierung oder einer verbindlichen Vorgabe der Organisation? → Sie wird nicht in das Framework kopiert, sondern über **Ebene B** (`devin-core-framework/framework/org-policies/`, Verweisblatt) eingebunden; der Core setzt sie höchstens operativ um.
...
7. **Verschärfungsprinzip:** Egal wo die Regel landet – sie darf höhere Ebenen nur konkretisieren oder verschärfen, nie lockern. Eine Lockerung ist nur als dokumentierte Ausnahme (`devin-core-framework/governance/EXCEPTION_PROCESS.md`) oder als Änderung der höheren Ebene selbst möglich.
