...
|---|---|
| **MUSS** / **DARF NICHT** | Zwingende Anforderung. Abweichung nur über den dokumentierten Ausnahmeprozess (`devin-core-framework/governance/EXCEPTION_PROCESS.md`). |
...
|---|---|---|---|
| A – Universelle Framework-Regeln | Governance, Datenschutz, Sicherheit, Qualität, Arbeitsmodell, Skill-Standard | `devin-core-framework/framework/core/` | Framework Owner |
| B – Organisationsweite Vorgaben | Richtlinien der Organisation (Einbindungspunkt, nicht Bestandteil des Frameworks) | `devin-core-framework/framework/org-policies/` | Organisation |
| C – Projektspezifische Konfiguration | Project Overlay | `project-overlay/` | Projekt (`<APPROVAL_ROLE>`) |
| D – Rollen- oder technologiebezogene Erweiterungen | Role Packs, Technology Packs | `devin-core-framework/framework/role-packs/`, `devin-core-framework/framework/tech-packs/` | Modul-Owner |
...
Inhalte einer Ebene DÜRFEN NICHT in eine andere Ebene geschrieben werden. Ein Projektwechsel darf ausschließlich Ebene C betreffen und DARF NICHT Änderungen an Ebene A erzwingen (Entscheidungsbaum `devin-core-framework/decision-trees/06-rule-placement.md`).
