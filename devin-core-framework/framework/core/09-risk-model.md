...
1. Jede Aufgabe, die an Devin gegeben wird, MUSS vor Beginn einer Kontrollstufe zugeordnet werden (Preflight-Check, `devin-core-framework/checklists/01-preflight.md`).
...
| R9 | Einführung externer Abhängigkeiten | keine | Aktualisierung einer bestehenden Abhängigkeit (Patch/Minor) | neue Abhängigkeit oder Major-Update (Checkliste `devin-core-framework/checklists/07-new-dependency.md`) |
...
| Dokumentationsumfang | Devin-Nutzungsvermerk im Merge Request (Kurzform, `devin-core-framework/templates/MR_AI_DISCLOSURE.md`) | zusätzlich: Plan, Fundstellenliste, Ergebnisbericht mit Abweichungen und Restrisiken | zusätzlich: vollständiges Sitzungsprotokoll (Prompts, Freigaben, ausgeführte Befehle), Entscheidungsvermerk der Freigabe |
