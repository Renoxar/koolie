...
**Warum falsch:** Der Skill hat weder `edit` noch `exec`; Fix, Testlöschung und Testlauf verletzen M1 (`devin-core-framework/framework/core/05-working-model.md`), das Verbot der Testabschwächung (`<ROOT_INSTRUCTION_FILE>` Abschnitt 8) und die Aufgabenteilung mit `fw-bugfix-prepare`. „Eindeutig" ersetzt die begründete Konfidenz und übergeht die fachliche Frage nach zulässigen leeren Bestellungen (P3). Die Aussage über die verursachende Person ist eine Personenbewertung (V7) und gehört in keine Fehleranalyse.
