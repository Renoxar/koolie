...
| K-07 | Existenz einer organisationsweiten KI-Richtlinie | mittel | Inhalt der Ebene B (organisationsweite Vorgaben) | Organisation | offen: Ebene B als Einbindungspunkt vorgesehen (`devin-core-framework/framework/org-policies/`) |
| K-08 | Zwei abweichende Fassungen der Prioritätshierarchie im Auftrag (7-stufig mit kombinierten Rollen-/Technologiepaketen vs. 8-stufig mit Technology Packs vor Role Packs) | hoch | Konfliktauflösung zwischen Regelebenen | Framework Owner | entschieden (Vorschlag): 8-stufige Fassung übernommen, ergänzt um das Verschärfungsprinzip; Begründung in `devin-core-framework/governance/PRIORITY_HIERARCHY.md` |
...
| D-02 | Regeln werden in zwei Formen gepflegt: kanonische Langform in `devin-core-framework/framework/` (tool-neutral) und kompakte Laufzeitform in `AGENTS.md` und `.devin/rules/` (Devin-spezifisch) | Tool Independence und Least Context; die Laufzeitform verweist auf die Langform | Nur Laufzeitform | entschieden (Vorschlag) | 2026-09-01 |
