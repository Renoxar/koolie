...
| `{kontrollstufe}` | MUSS | K1 | aus dem Preflight (`devin-core-framework/checklists/01-preflight.md`) |
...
Betriebsmodus: M5 Documentation Support (devin-core-framework/framework/core/05-working-model.md): Schreiben nur in <DOC_PATHS>; kein Quellcode, keine Konfiguration; Befehle nur lesende Git-Befehle.
...
- [ ] Prüfung auf vertrauliche Inhalte vor Übernahme oder Ablage in `<DOCUMENTATION_PLATFORM>` (`devin-core-framework/checklists/02-privacy-context.md`).
- [ ] Konventionen und Glossarbegriffe eingehalten; Beispiele als synthetisch gekennzeichnet.
- [ ] Review über den regulären Prozess (`devin-core-framework/checklists/08-merge-request.md`), Devin-Nutzungsvermerk enthalten.
