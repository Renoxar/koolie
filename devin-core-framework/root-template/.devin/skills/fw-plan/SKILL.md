...
- **Zweck:** Erarbeitet vor jeder Modifikation einen umsetzbaren, prüfbaren Änderungsplan exakt nach `devin-core-framework/templates/PLAN_TEMPLATE.md`: Ziel und Akzeptanzkriterien, Ist-Zustand mit Fundstellen, gekennzeichnete Annahmen und offene Fragen, bewertete Optionen (mindestens zwei bei Stufe mittel und hoch), kleine einzeln prüfbare Schritte mit Prüfung je Schritt, Teststrategie, Risiken und Gegenmaßnahmen, Rollback, Abbruchkriterien und Freigabeerfordernis. Der Plan ist Grundlage der Planbestätigung (Schritt 9 des Standardarbeitsablaufs) und der späteren Umsetzung mit `fw-change-small`, `fw-tests` oder `fw-docs-update`.
...
3. Aufgabenbeschreibung bereinigt (K2 gemäß `devin-core-framework/framework/core/02-privacy.md` Abschnitt 3.3 und 3.4) mit Akzeptanzkriterien.
...
**Zulässige Kontextquellen:** Quellcode, Tests und Schnittstellenbeschreibungen in `<ALLOWED_PATHS>` und `<READ_ONLY_PATHS>`; Dokumentation in `<DOC_PATHS>`; Overlay-Dokumente der Klasse K1 laut Manifest (Architektur, Conventions, Definition of Done); Analyseergebnis aus der Sitzung; `devin-core-framework/templates/PLAN_TEMPLATE.md`.

**Ausgeschlossene Informationen:** K3 gemäß `devin-core-framework/framework/core/02-privacy.md`; `<EXCLUDED_PATHS>`; Ticket-Kommentare und Anhänge; Werte aus Konfigurations- und Umgebungsdateien; Produktions- und Infrastrukturdetails.
...
5. Optionen bewerten (Plan-Abschnitt 4): bei Stufe mittel und hoch mindestens zwei Optionen, bei niedrig KANN eine genügen (mit Begründung); Kriterien Risiko, Aufwand, Reversibilität, Konsistenz mit den Architekturvorgaben des Overlays (Fundstelle im Overlay-Dokument); Empfehlung ausschließlich als Vorschlag; Optionen mit neuer Abhängigkeit als solche kennzeichnen (V3, `devin-core-framework/checklists/07-new-dependency.md`); Optionen, die die Delegationsverbotsliste berühren, mit V-Nummer ausweisen und nicht empfehlen.
...
4. Jede Planänderung nach Bestätigung erfordert eine erneute Bestätigung (`devin-core-framework/framework/core/05-working-model.md`, M2). Vor dem ersten Umsetzungsschritt `devin-core-framework/checklists/03-before-code-change.md` abarbeiten.
