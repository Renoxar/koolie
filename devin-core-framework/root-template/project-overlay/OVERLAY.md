...
<!-- AUSFÜLLHINWEISE (vor Aktivierung entfernen oder belassen – sie sind für Devin unschädlich):
     - Diese Datei ist die einzige Stelle für projektspezifische Vorgaben (Ebene 4). Der Framework Core
       ...
       und die Laufzeitfassung .devin/rules/20-project-overlay.md aus.
     ...
     - Das Overlay darf Framework-Regeln konkretisieren oder verschärfen, nie lockern
       (devin-core-framework/governance/PRIORITY_HIERARCHY.md).
     - Nach jeder Änderung: python3 devin-core-framework/tests/scripts/validate-framework.py ausführen und die Laufzeitfassung
       .devin/rules/20-project-overlay.md synchron halten. -->
...
| Overlay-Status | `<TBD: aktiv / inaktiv>` | `aktiv` erst nach Abschluss der Checkliste `devin-core-framework/checklists/10-project-adoption.md` |
| Overlay-Version | `<TBD: 0.1.0>` | Semantic Versioning, unabhängig von der Framework-Version |
| Kompatible Framework-Version | `<TBD: z. B. 0.1.x>` | aus `devin-core-framework/VERSION` des Frameworks |
...
| Ergebnis Datenschutz- und Vertragsprüfung | `<TBD: Referenz oder „ausstehend">` | siehe `devin-core-framework/framework/core/02-privacy.md` Abschnitt 1 |
...
```text
<REPOSITORY_NAME>/
├── AGENTS.md                      # Framework (nicht ändern)
├── .devin/                        # Framework-Laufzeitschicht + Overlay-Laufzeitfassung
├── devin-core-framework/framework/                     # Framework Core, Packs (nicht ändern)
...
| Ausgeschlossene Pfade (weder lesen noch ändern) | `<EXCLUDED_PATHS>` | `<TBD: z. B. deploy/**, infra/**, config/prod/**, **/fixtures/real/**>` | zusätzlich zu den festen Framework-Ausschlüssen (Secrets, `.devin/`, `AGENTS.md`, `project-overlay/`, `devin-core-framework/framework/`) |
...
| CI/CD-Konfiguration | `<CI_CONFIG_PATHS>` | `<TBD: z. B. .gitlab-ci.yml, .github/workflows/**, Jenkinsfile>` | wird in `.devin/config.json` als `Write`-deny eingetragen |
| Quality-Gate-Konfiguration | `<QUALITY_GATE_CONFIG_PATHS>` | `<TBD: z. B. Linter-, Coverage-, Analyse-Konfigurationsdateien>` | `Write`-deny |

Diese Werte werden in `.devin/config.json` und in `.devin/rules/20-project-overlay.md` übernommen. Bei Widerspruch gilt die restriktivere Angabe.
...
| Zweck | Platzhalter | Befehl | Freigegeben für Modus | Freigabestufe in `.devin/config.json` |
|---|---|---|---|---|
...
| Aktivierte Technology Packs | `<TBD: Liste, z. B. tech-<name> Version>` | jeweils Laufzeitfassung `.devin/rules/40-tech-<name>.md` |
...
| Merge-Request-Vorlage | `<MR_TEMPLATE_PATH>` | `<TBD>`; enthält den Devin-Nutzungsvermerk (`devin-core-framework/templates/MR_AI_DISCLOSURE.md`) |
...
Zusätzlich zur projektweiten Definition of Done (`<TBD: Pfad>`) gilt `devin-core-framework/framework/core/04-quality.md` Abschnitt 3 (Ergebnisbericht, lokale Prüfungen, Selbstreview, Nutzungsvermerk, offene Punkte sichtbar). Projektspezifische Ergänzungen: `<TBD>`.
...
| Freigegebene MCP-Server | – | `<TBD: Liste mit Zweck und Berechtigungsumfang oder „keine">` | Eintrag in `.devin/mcp_config.json` erst nach Freigabe; Standard ask |
| Freigegebene externe Domains (Fetch) | K0 | `<TBD: Liste oder „keine">` | `Fetch(domain:...)`-Regeln in `.devin/config.json` |
...
Zusätzlich zu den festen K3-Kategorien (`devin-core-framework/framework/core/02-privacy.md` Abschnitt 2.1):

| Ausgeschlossen | Wo typischerweise anzutreffen | Technische Sperre |
|---|---|---|
| `<TBD: z. B. Fachdaten realer Vorgänge>` | `<TBD: Verzeichnisse>` | `Read`-deny in `.devin/config.json` |
...
| Reviewerinnen und Reviewer | – | Review nach `devin-core-framework/framework/core/07-review-rules.md` |
...
| Stufe (aus `devin-core-framework/framework/core/10-error-escalation.md`) | Erreichbar über | Reaktionszeit (projektspezifisch) |
|---|---|---|
...
| E4 Framework-Mangel | `<FRAMEWORK_OWNER>` über `devin-core-framework/governance/FEEDBACK_PROCESS.md` | `<TBD>` |
...
Regeln: Projekt-Skills folgen dem Skill-Standard (`devin-core-framework/framework/core/08-skill-conventions.md`), tragen das Präfix `prj-`, liegen unter `.devin/skills/prj-<name>/` und DÜRFEN Framework-Skills ergänzen, aber deren Prüfschritte nicht entfernen.
...
Ausnahmen folgen `devin-core-framework/governance/EXCEPTION_PROCESS.md`. Ausnahmen von Delegationsverboten (V1–V12) und von der K3-Definition sind nicht zulässig.
...
3. Ladeverhalten:
   - `summary`: Kernaussagen (maximal zehn Zeilen) werden in `.devin/rules/20-project-overlay.md` übernommen (immer aktiv, nur K1).
   - `on-demand`: Devin liest das Dokument, wenn die Aufgabe es erfordert; K2-Dokumente nur nach Freigabe in der Aufgabe.
   - `rule`: Das Projekt legt eine zusätzliche Overlay-Regeldatei `.devin/rules/2N-overlay-<name>.md` (N = 1–9) mit `trigger: model_decision` oder `glob` an, die die Kernregeln des Dokuments enthält und auf das Dokument verweist (unter 12.000 Zeichen).
...
Der Status wird erst auf `aktiv` gesetzt, wenn `devin-core-framework/checklists/10-project-adoption.md` vollständig abgearbeitet ist und `python3 devin-core-framework/tests/scripts/validate-framework.py --strict-overlay` ohne Befund durchläuft.
