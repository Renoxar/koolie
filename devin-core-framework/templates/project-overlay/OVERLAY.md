...
| Ausgeschlossene Pfade (weder lesen noch ändern) | `<EXCLUDED_PATHS>` | `<TBD: z. B. deploy/**, infra/**, config/prod/**, **/fixtures/real/**>` | zusätzlich zu den festen Framework-Ausschlüssen (Secrets, `<RUNTIME_DIR>/`, `<ROOT_INSTRUCTION_FILE>`, `project-overlay/`, `devin-core-framework/framework/`) |
...
Regeln: Projekt-Skills folgen dem Skill-Standard (`devin-core-framework/framework/core/08-skill-conventions.md`), tragen das Präfix `prj-`, liegen unter `<SKILLS_DIR>/prj-<name>/` und DÜRFEN Framework-Skills ergänzen, aber deren Prüfschritte nicht entfernen.
