...
|---|---|
| **MUSS** / **DARF NICHT** | Zwingende Anforderung. Abweichung nur über den dokumentierten Ausnahmeprozess (`governance/EXCEPTION_PROCESS.md`). |
...
Governance-, Qualitäts- und Strukturprinzipien sind werkzeugneutral formuliert und in `framework/` kanonisch abgelegt. Die Konkretisierung für Devin Desktop erfolgt ausschließlich in der Laufzeitschicht (`AGENTS.md`, `.devin/`). Ein Werkzeugwechsel darf nur die Laufzeitschicht betreffen.
...
|---|---|---|---|
| A – Universelle Framework-Regeln | Governance, Datenschutz, Sicherheit, Qualität, Arbeitsmodell, Skill-Standard | `framework/core/` | Framework Owner |
| B – Organisationsweite Vorgaben | Richtlinien der Organisation (Einbindungspunkt, nicht Bestandteil des Frameworks) | `framework/org-policies/` | Organisation |
| C – Projektspezifische Konfiguration | Project Overlay | `project-overlay/` | Projekt (`<APPROVAL_ROLE>`) |
| D – Rollen- oder technologiebezogene Erweiterungen | Role Packs, Technology Packs | `framework/role-packs/`, `framework/tech-packs/` | Modul-Owner |
...
Inhalte einer Ebene DÜRFEN NICHT in eine andere Ebene geschrieben werden. Ein Projektwechsel darf ausschließlich Ebene C betreffen und DARF NICHT Änderungen an Ebene A erzwingen (Entscheidungsbaum `decision-trees/06-rule-placement.md`).
