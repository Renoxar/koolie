...
2. Das Framework ergänzt bestehende Entwicklungs-, Review- und Freigabeprozesse. Es ersetzt sie nicht und hat im Konfliktfall keinen Vorrang vor rechtlichen, regulatorischen oder organisationsweiten Vorgaben (`governance/PRIORITY_HIERARCHY.md`).
...
| Modul-Owner | Fachliche und technische Verantwortung für ein Role Pack, Technology Pack oder eine Skill-Gruppe | je Modul im `OWNERS.md` |
...
Die detaillierte RACI-Zuordnung liegt in `governance/RACI.md`.

## 3. Änderungsgrundsätze (normativ)

1. Änderungen am Framework Core erfolgen ausschließlich über Änderungsanträge (`governance/CHANGE_REQUEST_TEMPLATE.md`) und Releases (`governance/RELEASE_PROCESS.md`).
2. Änderungen am Project Overlay erfolgen über den Prozess des Projekts, MÜSSEN aber die Validierung (`tests/scripts/validate-framework.py`) bestehen und DÜRFEN NICHT Core-Dateien verändern.
3. Jede Änderung ist im `CHANGELOG.md` (Framework) beziehungsweise im Overlay-Änderungsverlauf dokumentiert.
