...
1. Die Einstufung erfolgt durch den Menschen vor der Bereitstellung (Entscheidungsbaum `decision-trees/01-context-allowed.md`).
...
4. Vorfall im Framework-Betrieb erfassen (`governance/INCIDENT_HANDLING.md`) und Ursache im nächsten Review-Zyklus adressieren (zum Beispiel fehlende `deny`-Regel).
...
| Prüfung von Werkzeugaufrufen auf Secrets | Hook `PreToolUse` mit Skript, das Schreib- und Ausführungsanfragen auf Secret-Muster prüft und blockiert (`tests/scripts/hook-check-secrets.py`) | `[DOK]` (Hook-Mechanismus), `[EMPF]` (Skript) |
