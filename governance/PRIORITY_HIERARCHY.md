...
| 2 | Organisationsweite Richtlinien | `framework/org-policies/` (Einbindungspunkt) |
| 3 | Framework Core | `AGENTS.md`, `framework/core/`, `.devin/rules/00-*, 10-*, 15-*`, `.devin/config.json` (Kernregeln) |
| 4 | Project Overlay | `project-overlay/`, `.devin/rules/2N-*` |
| 5 | Technology Packs | `framework/tech-packs/`, `.devin/rules/40-*` |
| 6 | Role Packs | `framework/role-packs/`, `.devin/rules/30-*` |
...
4. **Delegationsverbote und K3 sind ebenenfest:** V1–V12 und die K3-Definition können von keiner tieferen Ebene und keiner Nutzeranweisung außer Kraft gesetzt werden; auch der Ausnahmeprozess deckt sie nicht (`governance/EXCEPTION_PROCESS.md`).
