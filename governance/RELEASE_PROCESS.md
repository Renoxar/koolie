...
1. Das Framework folgt Semantic Versioning (`VERSION`, `CHANGELOG.md`): **MAJOR** bei Struktur- oder Hierarchieänderungen, die Overlays anpassen müssen; **MINOR** bei neuen Modulen, Skills oder Regeln ohne Overlay-Bruch; **PATCH** bei Korrekturen und Formulierungen.
...
3. Angenommene Anträge werden umgesetzt, validiert (`tests/scripts/validate-framework.py`), im Testkatalog abgedeckt und im Decision Log sowie `CHANGELOG.md` dokumentiert.

## 4. Release-Prozess (normativ)

1. Release-Vorbereitung nach `checklists/11-framework-release.md` (Konsistenz, Projektneutralität, Produktstand, Testkatalog).
...
4. **Werkzeugwechsel:** Dank Tool Independence (P8) beschränkt sich ein Wechsel oder Parallelbetrieb eines anderen KI-Werkzeugs auf eine neue Laufzeitschicht (Analog zu `.devin/`); die kanonischen Regeln in `framework/` bleiben unverändert. Ein solcher Schritt ist ein MAJOR-Release.
