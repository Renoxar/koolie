...
Stellt vor dem ersten Prompt sicher, dass die Aufgabe delegierbar, richtig eingestuft, sauber abgegrenzt und der Kontext zulässig ist. Ohne abgeschlossenen Preflight-Check beginnt keine KI-Aufgabe (`leitwerk-core/framework/core/05-working-model.md`, Schritt 1–4).
...
- [ ] **MUSS** Kontrollstufe nach `leitwerk-core/framework/core/09-risk-model.md` bestimmt (alle Faktoren R1–R13 durchgegangen, Maximumprinzip) und der auslösende Faktor notiert (zum Beispiel „mittel wegen R8").
- [ ] **MUSS** Betriebsmodus M1–M5 festgelegt (`leitwerk-core/decision-trees/03-analyze-or-modify.md`).
...
- [ ] **MUSS** Overlay-Status ist `aktiv` (Ausnahme: Onboarding-Übung auf dem Übungsrepository; am Quellrepositorium des Frameworks gilt statt dessen `leitwerk-core/governance/FRAMEWORK_DEV_PROFILE.md`, das **keine** technische Berechtigung erteilt und dessen Geltung der KI-Client nicht selbst feststellt – D-253).
