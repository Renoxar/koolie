...
| S3 | Werkzeugbeschränkung je Skill | **Keiner.** `allowed-tools` ist eine **Vorabfreigabe** für den aufrufenden Turn, keine abschließende Werkzeugliste – andere Werkzeuge bleiben verfügbar und unterliegen ihren regulären Berechtigungen. Das Feld `permissions` der Quelle kennt dieser Client für Skills nicht; es steht im Manifest unter `drop_fields` und benennt dort seinen Ersatz. **Ersatz: die globale Berechtigungsschicht samt Schutz-Hook** – sie wirkt unabhängig vom Skill und hielt am 2026-09-12 auch im untersagten Betriebsmodus, ist aber **weniger** als eine Beschränkung je Skill: Ein Analyseskill kann schreiben, soweit die globalen Regeln es zulassen | `[NICHT ABBILDBAR]` | **Gemessen am 2026-09-12** (`tests/protocols/2026-09-12-B01-allowed-tools.md`): Ein Skill mit `allowed-tools: Read, Grep, Glob` **konnte schreiben**. Die frühere Einstufung `[TECHNISCH]` mit Beleg `[DOK]` ist damit widerlegt – und der Dokumentationsbeleg trug eine Aussage, die die Dokumentation so nicht macht (Befund **B01** des Reviews, `CR-2026-050`, D-50). Die Dokumentation nennt `disallowed-tools` als gesonderten Mechanismus; **nicht erhoben, deshalb hier nicht zugesagt** `<VERIFY AGAINST CURRENT CLIENT DOCUMENTATION>`. Nach D-41 ist S3 eine **Fähigkeitszusage**; ihr Ausfall sperrt die Inbetriebnahme nicht |
