...
| Verbindliche Zielversion | `3.9.x`, Agent-CLI `3000.10.x` (D-112). Die Spanne ist der Geltungsbereich dieses Packs; der gemessene Punktwert steht in der Zeile darunter (D-113) |
| Geprüfte Clientversion | 3.9.19 (Agent-CLI 3000.10.21) – gemessen am 2026-09-16, `leitwerk-core/tests/protocols/2026-09-16-AP2-zielversion-devin-desktop.md` Läufe E1 und E2 |
| Datum der Prüfung | 2026-09-16 – Erhebung der Pfadabbildung an einer Installation. Die Belege zur Berechtigungsdatei stammen vom 2026-09-11 und vom 2026-09-14 und liegen in der Zielspanne (D-114) |

> **Teilweise belegt (Stand 0.53.0).** Die Spalte „Einstufung" nennt die **vorgesehene** Durchsetzungstiefe, die Spalte „Beleg" ihren Nachweisstand: `[DOK]` = in der Herstellerdokumentation beschrieben, `[EMPF]` = Empfehlung des Frameworks, VERIFY-Marker = gegen eine Installation innerhalb der verbindlichen Zielspanne zu prüfen. **Die Zielspanne ist mit D-112 festgelegt, und die Pfadabbildung ist gegen sie erhoben** (`leitwerk-core/tests/protocols/2026-09-16-AP2-zielversion-devin-desktop.md`). Beobachtet sind H1, H2, R5, R6 und S5. **Fünf Zeilen der Fähigkeitsmatrix tragen weiterhin einen offenen Marker** – S3, B3, B10, A1 und X2; sie brauchen einen Sitzungstest und sind der Rest von `AP2`.
...
Die Regelmenge liegt werkzeugneutral im Kern (`leitwerk-core/framework/runtime/permissions.json`, `hooks.json`) und wird bei der Installation in die Werkzeuge dieses Clients übersetzt (D-18). Was dabei abgebildet wird, steht maschinenlesbar im `manifest.json`; diese Tabelle ist die menschenlesbare Fassung.
...
2026-09-12 (K-28, `leitwerk-core/tests/protocols/2026-09-12-erhebungen-K28-S5-B9-bypass.md`):
...
| X2 | Art und Ort der Codebasis-Indexierung bekannt | Kein Mechanismus zur Steuerung bekannt | `[NICHT ABBILDBAR]` | `<VERIFY AGAINST CURRENT DEVIN DOCUMENTATION>` (K-20). **Kein Ersatz durch das Framework:** Was ein Client indexiert und wohin er es gibt, ist von außen nicht zu beobachten – weder die Installation noch der Validator sehen es. Die Zusage entfällt für diesen Client; K-20 hält die Frage offen. Nach D-41 eine **Fähigkeitszusage**, keine Kernzusage |
...
**Belegstand:** **9 der 36 Zeilen** tragen einen offenen VERIFY-Marker – S3, B3, B10, A1 und X2 unmittelbar, B4, B5, B6 und B8 über den Verweis „wie B3". **Die Zahl stand hier bis 0.35.0 auf „8" und die Aufzählung nannte B10 nicht**, obwohl es seinen Marker seit 0.33.0 trägt (`CR-2026-058`, Befund 6). Nachgezählt sind allein die **unmittelbaren** Marker – die vier über den Verweis mitgezählten sind eine Ermessensgrenze und werden von keiner Prüfung nachgerechnet. Keine Einstufung ist gegen eine Installation geprüft; **beobachtet** sind H1, H2 (AP2) sowie R5, R6 und S5 (Erhebungen vom 2026-09-11), und drei Marker sind mit diesem Release aufgelöst worden: R4 und B9 zum Schlechteren (die Messung widerlegt die Zusage), M2 durch den richtigen Mechanismus.
...
Ergibt die Prüfung, dass eine der sechs Zusagen nicht technisch durchgesetzt wird, ist sie hier einzutragen und nach der Regel in `leitwerk-core/clients/README.md` Abschnitt 4 durch `<SECURITY_CONTACT>` freizugeben.
...
```text
python leitwerk-core/install.py --client devin-desktop
