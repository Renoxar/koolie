...
| B3 | Secret-Dateien per Pfadmuster lesegeschützt | ja | Verweigerungsregeln auf `.env`, `**/*.pem`, `**/secrets/**` und weitere. **Je Kanal:** direktes Lesen **wirkt** (Regel und Hook); Shell **wirkt** über den Schutz-Hook; **Suche entfällt – dieser Client führt kein eigenes Suchwerkzeug** (siehe Verbtabelle, Abschnitt 3; im Manifest als `hook_tools_absent` erklärt); Unterprozess **nicht nachgewiesen**. Im untersagten Betriebsmodus greift die Regel nicht und allein der Hook trägt (`AP2-DD-12`, D-35) | `[TECHNISCH]` für Lesen und Shell; `[TEXTUELL]` für den Unterprozess | Mechanismus `[DOK]`; Muster-Semantik `<VERIFY AGAINST CURRENT DEVIN DOCUMENTATION>`. **Reichweite je Kanal**: Der Befund B04 ist am Schutz-Hook gemessen (`tests/protocols/2026-09-12-B04-B05-gegenpruefung.md`); der Hook ist bei beiden Packs derselbe, die Berechtigungsschicht nicht |
