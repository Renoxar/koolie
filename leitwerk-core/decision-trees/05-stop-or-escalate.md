...
2. **Der Mensch ordnet die Eskalationsstufe zu:**
   ...
   - **E3 – Sicherheit/Datenschutz:** S3, S6, sowie S2, wenn die Bereitstellung bereits erfolgt ist → `<SECURITY_CONTACT>`, bei Personenbezug `<DATA_PROTECTION_CONTACT>`; Prozess der Organisation; Erfassung in `leitwerk-core/governance/INCIDENT_HANDLING.md`. Bei S3: Sitzung beenden, Secrets als kompromittiert behandeln und rotieren lassen (`leitwerk-core/framework/core/02-privacy.md` Abschnitt 5).
...
3. **Wiederanlauf:** neue Sitzung, angepasster Scope, ausdrücklicher Verweis auf den Abbruchgrund; halbe Änderungen vorher vollständig verwerfen oder als abgegrenzten Zwischenstand committen (`leitwerk-core/framework/core/10-error-escalation.md` Abschnitt 4).
...
- Für Metriken zählt jede Eskalation mit Stufe und Grund (Pilotauswertung, `leitwerk-core/pilot/METRICS.md`).
