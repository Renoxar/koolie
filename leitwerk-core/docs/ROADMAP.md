...
> 🟢 **DER RESTWEG BIS 1.0.0, ALS ZAHL STATT ALS GEFÜHL** (neu mit 0.77.0). Kriterium 2 ist seit dem Plan von `0.56.0` **111 → 105 → 100 → 93 → 92 → 85 → 74 → 56 → 38 → 30 → 32 → 19** gegangen. 🟢 **Der Nachlauf von Bündel 4 hat seine Zusage eingelöst: dreizehn von dreizehn Zellen abgenommen, 32 → 19** (`0.80.0`). **92 von 111 Zellen sind zu; es bleiben Bündel 5 (19 → 4) und die vier Sammelzellen (4 → 0).** 🔴 **Der letzte Schritt ist der erste AUFWÄRTS in dieser Kette, und er ist gewollt** (`0.79.2`, D-227): `0.79.0` hat `fw-review-support` und `fw-mr-description` in demselben Commit gehoben, der zwei ihrer Zellen abgenommen hat – beide stehen damit auf einer Fassung, die es nicht mehr gibt, und gehen zurück auf `offen`. **79 von 111 Zellen sind zu.** ⚠️ **Die Kette war neun Schritte lang monoton, und genau das hat die Frage verdeckt:** Ein Zähler, der nur fallen kann, sagt nichts darüber, ob seine Nullen noch gelten. 🔴 **Und der jüngste Schritt ist der kleinste seit `0.68.0` und der einzige, der hinter seiner Zusage zurückblieb:** Bündel 4 war mit 38 → 19 geplant und hat 38 → 30 gebracht, weil der Meßbaum auf `main` stand (D-218). Was bleibt, sind **ein Nachlauf und zwei Meßtage** (Nachlauf Bündel 4: 32 → 19, Bündel 5: 19 → 4, die vier Sammelzellen: 4 → 0), **zwei Sitzungen für Kriterium 1** und **eine für die Umbenennung**. ⚠️ **Und die ehrliche Hälfte derselben Zahl:** Bündel 4 hat **vier** Vorbereitungsreleases gebraucht (`0.74.1`, `0.75.0`, `0.76.0`, `0.77.0`) gegenüber **je zwei** bei Bündel 1 bis 3 – doppelt so viel Vorlauf, und zwei davon kamen aus `K-77`, also aus dem Meßtag von Bündel 3 selbst. **Was die Einschübe nicht kosten:** Von den 23 Releases seit `0.56.0` haben **sechzehn kein Kontingent** gebraucht; alle vier Vorbereitungsreleases von Bündel 4 sind darunter. Ein Release ist hier ein Buchungsvorgang, keine Messung – **die knappe Ressource ist der Meßtag, und die Einschübe verbrauchen sie nicht.** `0.76.0` hat für null Dollar gefunden, daß zwölf Zellen gegen einen Baum ohne Git-Repositorium gelaufen wären.
>
> 🟢 **MIT `0.87.0` STEHEN ALLE VIER ZÄHLBAREN KRITERIEN AUF NULL** (`CR-2026-121`, D-291 bis D-297). Kriterium 1 ist **18 → 0** gegangen: sechzehn Fundstellen sachlich aufgelöst, zwei umgewidmet (dieselbe Frage), beide Registerzeilen entfallen. ⚠️ **Das heißt nicht `1.0.0`-reif**, und Prüfung 46 behauptet es auch nicht (E8 von `CR-2026-070`) – sie rechnet vier Zahlen aus und hält sie gegen diese Zeile. **Was zwischen diesem Stand und `1.0.0` liegt, steht unten im Plan:** die Umbenennung (`~0.88.0`) und `AP11`. 🔴 **Und die Zahl ändert ihre Bedeutung:** Kriterium 1 ist ab jetzt eine **Rückfallsperre** und kein Arbeitsvorrat (D-293) – was sie meldet, ist die Wiedereinführung einer abgeschafften Form.
...
| ~~**0.87.0**~~ ✅ | *dieses Release:* 🟢 **DIE MARKERFORM SELBST IST ABGESCHAFFT – KRITERIUM 1 STEHT AUF NULL** (`CR-2026-121`, **D-291** bis **D-297**, `K-98` neu). **18 Fundstellen in 14 Dateien**, dazu **sechs weitere in versionierten Trägern, die Prüfung 46 nie gesehen hat** (`README.md`, fünf Quellen unter `build/doc/`). **Sechzehn sind sachlich aufgelöst** – fünf durch die Messungen von `0.86.0` (`S3`/D-287, `B3`/D-277, `A1`/D-284), drei durch einen Verweis auf die Fähigkeitsmatrix, fünf waren reine Nennungen, drei sind Register- und Glossarzeilen –, **zwei sind umgewidmet und dieselbe Frage:** `X2` und die Nachweiszelle von `K-20` sagen `BELEG OFFEN (dauerhaft)`, und die Frage bleibt offen. 🔴 **Der Befund, der den Schritt trägt:** Der Marker verband den **Belegstand** einer Aussage mit einer **Frist** – *„vor Version 1.0.0"* –, und für eine dauerhaft nicht beobachtbare Frage war sie nie einlösbar. 🔴 **Fünf Befunde fielen vor dem ersten Handgriff**, darunter drei überholte Belegstände beider Packs (D-297) und der Vorbehalt von Prüfung 34, der ohne Nachfolgeform toter Code geworden wäre (D-296) | Kriterium 1: **18 → 0** ✅ | ja |
| **~0.88.0** | 🔴 **Die Umbenennung auf `Koolie`** – 🟢 **VORZIEHUNG BEANTRAGT UND ABGELEHNT** (`CR-2026-119`, **D-269**, 2026-09-22): Sie bleibt **hinter** dem Rest von `AP2`, D-127 gilt unverändert. *Die tragende Hälfte der Begründung von D-127 ist zwar entfallen (**105 → 0** offene Ergebniszellen), die andere nicht – und der Preis der Vorziehung, ein wiederholter Vorbedingungsdurchgang der `AP2`-Sitzung auf dem umbenannten Baum, wird nicht bezahlt.* 🟢 **Der Umfang ist gemessen und liegt bereit** – 490 von 494 Dateien wandern mit einem `git mv`, die Arbeitsfläche des Textlaufs ist **925 Fundstellen in 128 Dateien** (die Chronik wandert nicht mit, **D-273**, und verträgt sich mit Prüfung 12), 43 Träger führen `<CORE_DIR>`, die Werkzeuge nennen den Namen **304**mal, und die Migrationsfläche beider übernehmenden Projekte ist **30 Dateien mit 141 Nennungen** (am 2026-09-22 nachgezählt; der Antrag nannte 27 – die `.gitignore` beider Projekte fehlte. `K-50`, **D-270**: Migrationshinweis statt maschinellem Pfad). ⚠️ **Die Zahlen sind gegen `0.85.0` gezählt und altern** – wer den Lauf fährt, zählt sie nach; der Ablauf steht in zehn Schritten in `CR-2026-119` Abschnitt 5. 🆕 **Der Lauf baut Prüfung 75** (**D-271**), die einen Restbestand des alten Namens meldet – *sonst ist „der Name ist weg" eine Behauptung ohne Prüfung* –, und das Repositorium wird **nach** dem Merge umbenannt (**D-274**). (D-125, Zeitpunkt nach D-127). `leitwerk-core/` wird `koolie-core/`, `<CORE_DIR>` ändert seinen Wert, das Repositorium seinen Namen. **Hier, weil alle Messungen abgeschlossen sind und `AP11` noch nicht gelaufen ist** – sonst trügen Hauptdokument und Word-Fassung den alten Namen und müssten zweimal gebaut werden. 🆕 **Und derselbe Vorgang zieht das Framework in ein eigenes Unterverzeichnis des Zielprojekts** (`CR-2026-098`): Der Kern und `project-overlay/` liegen künftig unter `.koolie/` statt im Wurzelverzeichnis. **Hier, weil es sonst zweimal geschieht:** D-127 verschiebt das Kernverzeichnis ohnehin und ändert den Wert von `<CORE_DIR>` – wer beides trennt, faßt jede Fundstelle zweimal an und hebt jedes übernehmende Projekt zweimal. 🟢 **Die beiden fristgebundenen Fragen von `K-75` sind mit `0.85.2` entschieden** (**D-272**): **`.koolie/core/`** – mit Punkt, und der Kern heißt darunter `core/`. 🔴 **Der Wert bekommt damit erstmals einen Schrägstrich** (`.koolie/core`); jede Stelle, die ihn als einzelnes Verzeichnissegment behandelt, bricht. **Die übrigen fünf Entscheidungen von `K-75` gehören zu `1.3.0`** | – | nein |
| **~0.89.0** | **`AP11` Stabilisierung:** das Hauptdokument gegen den dann geltenden Stand setzen (es ist über vierzig Releases zurück), Word-Fassung bauen, Gegenzeichnung der zwölf offenen und fünf fehlenden Protokollabschnitte nachziehen, `CR-2026-029` und `-030` Abschnitt 6 nachtragen. **Erstmals vollständig unter dem neuen Namen.** 🔴 **Und hier fällt die einzige befristete Ausnahme von Prüfung 48:** `build/` ist von der Neutralitätsregel ausgenommen, weil es die Quellen des Hauptdokuments hält (D-128). Wer `AP11` fährt, streicht die Ausnahme in `docs/RUNTIME_GLOSSARY.md` und in `tests/scripts/validate-framework.py` und räumt die dann gemeldeten Fundstellen mit auf | – | nein |
