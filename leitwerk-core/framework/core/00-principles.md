...
|---|---|
| **MUSS** / **DARF NICHT** | Zwingende Anforderung. Abweichung nur über den dokumentierten Ausnahmeprozess (`leitwerk-core/governance/EXCEPTION_PROCESS.md`). |
...
Governance-, Qualitäts- und Strukturprinzipien sind werkzeugneutral formuliert und in `leitwerk-core/framework/` kanonisch abgelegt. Die Konkretisierung für einen konkreten KI-Client erfolgt ausschließlich in der Laufzeitschicht, also in der Wurzel-Anweisungsdatei und der Laufzeitablage (`leitwerk-core/docs/RUNTIME_GLOSSARY.md`). Ein Werkzeugwechsel darf nur die Laufzeitschicht betreffen; er wird über ein Client Pack abgebildet (`leitwerk-core/clients/README.md`).
...
|---|---|---|---|
| A – Universelle Framework-Regeln | Governance, Datenschutz, Sicherheit, Qualität, Arbeitsmodell, Skill-Standard | `leitwerk-core/framework/core/` | Framework Owner |
| B – Organisationsweite Vorgaben | Richtlinien der Organisation (Einbindungspunkt, nicht Bestandteil des Frameworks) | `leitwerk-core/framework/org-policies/` | Organisation |
| C – Projektspezifische Konfiguration | Project Overlay | `project-overlay/` | Projekt (`<APPROVAL_ROLE>`) |
| D – Rollen- oder technologiebezogene Erweiterungen | Role Packs, Technology Packs | `leitwerk-core/framework/role-packs/`, `leitwerk-core/framework/tech-packs/` | Modul-Owner |
...
Inhalte einer Ebene DÜRFEN NICHT in eine andere Ebene geschrieben werden. Ein Projektwechsel darf ausschließlich Ebene C betreffen und DARF NICHT Änderungen an Ebene A erzwingen (Entscheidungsbaum `leitwerk-core/decision-trees/06-rule-placement.md`).
