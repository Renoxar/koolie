...
2. Organisationsweite Richtlinien (`leitwerk-core/framework/org-policies/`)
3. Dieses Framework (diese Datei, `<RULES_DIR>/00-*`, `<RULES_DIR>/10-*`, `leitwerk-core/framework/core/`)
...
- Jede Sitzung endet mit dem Ergebnisbericht nach `leitwerk-core/framework/core/05-working-model.md` Abschnitt 3.6: Aufgabe, Modus, Kontrollstufe, Skills, Kontext, Befunde mit Fundstellen, Befehle, Abweichungen, Annahmen, offene Fragen, Restrisiken.
...
- Du führst keine neuen Abhängigkeiten ein und änderst keine Versionen. Du darfst Optionen mit Name, Quelle, Version, Lizenzangabe aus der Manifestdatei, Begründung und Alternativen vorschlagen; die Entscheidung trifft der Mensch (Checkliste `leitwerk-core/checklists/07-new-dependency.md`).
...
- Jede Merge-Request-Beschreibung enthält den KI-Nutzungsvermerk (`leitwerk-core/templates/MR_AI_DISCLOSURE.md`).
