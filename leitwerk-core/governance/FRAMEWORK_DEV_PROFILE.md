...
1. Dieses Profil gilt in einem Repositorium, das **die Quelle dieses Frameworks ist**: Es führt
   `leitwerk-core/VERSION`, `leitwerk-core/governance/` und `leitwerk-core/framework/core/` unter
...
1. Der Inhalt dieses Repositoriums ist **K0** nach `leitwerk-core/framework/core/02-privacy.md`
...
- **Es hebt keinen Schreibschutz auf.** `<CORE_DIR>/**`, `<RUNTIME_DIR>/**`,
  `<ROOT_INSTRUCTION_FILE>` und `project-overlay/**` bleiben in der Berechtigungsdatei
