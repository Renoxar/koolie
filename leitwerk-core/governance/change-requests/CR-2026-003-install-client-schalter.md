...
| Datum | 2026-09-10 |
| Betroffene Artefakte | verschoben: `leitwerk-core/root-template/` → `leitwerk-core/clients/devin-desktop/root-template/`; verschoben: `leitwerk-core/framework/client-packs/` → `leitwerk-core/clients/`; geändert: `leitwerk-core/install.py`, `README.md`, `leitwerk-core/docs/ADOPTION_GUIDE.md`, `leitwerk-core/OWNERS.md` |
...
`CR-2026-002` hat die Client Packs als Abbildungsschicht eingeführt, die Wurzelartefakte aber dort belassen, wo sie lagen: in `leitwerk-core/root-template/`. Damit blieb die Schicht Dokumentation ohne Wirkung – `install.py` kannte weiterhin genau eine, fest verdrahtete Vorlage. Ein zweites Client Pack hätte seine Artefakte nirgends unterbringen können.

## 2. Vorgeschlagene Änderung

1. **`root-template/` wandert unter das Client Pack**, dem es gehört: `leitwerk-core/clients/devin-desktop/root-template/`. Ein Client Pack besteht damit aus `CLIENT_PACK.md` (was der Client durchsetzt) und `root-template/` (was installiert wird).
