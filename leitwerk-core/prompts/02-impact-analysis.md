...
- Bereinigte Aufgabenbeschreibung: Titel, technische Beschreibung, Akzeptanzkriterien (K2, bereinigt nach `leitwerk-core/framework/core/02-privacy.md` Abschnitt 3.3 und 3.4).
...
- Unbereinigte Ticket-Exporte („hier ist der Export, analysiere das") – zuerst bereinigen (`leitwerk-core/checklists/02-privacy-context.md`).
...
Betriebsmodus: M1 Read-only Analysis (leitwerk-core/framework/core/05-working-model.md). Keine Datei erzeugen oder ändern, keine Befehle ausführen – auch keine Tests „zur Prüfung der Abdeckung".
...
2. Prüfe die Delegierbarkeit: Berührt die Änderung V1–V12 aus leitwerk-core/framework/core/09-risk-model.md Abschnitt 4 (Secrets, Produktionssysteme, Berechtigungen, Abhängigkeiten)? Kennzeichne die Berührung; die Analyse bleibt zulässig, die Umsetzung des Anteils ist nicht delegierbar.
...
- Abschnitt „Annahmen (gekennzeichnet) und offene Fragen"; Ergebnisbericht nach `leitwerk-core/framework/core/05-working-model.md` Abschnitt 3.6.

## 7. Prüfschritte

- [ ] Kontrollstufe eigenverantwortlich festgelegt und im Preflight dokumentiert (`leitwerk-core/checklists/01-preflight.md`); im Zweifel die höhere Stufe gewählt.
...
- [ ] Keine K2-Inhalte ohne Freigabe und keine K3-Inhalte im Bericht wiederholt (`leitwerk-core/checklists/02-privacy-context.md`).
- [ ] Bericht bei Stufe mittel und hoch als Grundlage des Plans referenziert und mit dem Ergebnisbericht abgelegt (`<TBD: Ablageort für Ergebnisberichte>`); vor einer Änderung `leitwerk-core/checklists/03-before-code-change.md`.
