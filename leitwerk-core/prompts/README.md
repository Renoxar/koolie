...
1. Liegt für eine Aufgabe ein Skill vor, MUSS er bevorzugt verwendet werden (`leitwerk-core/framework/core/06-prompting-rules.md`, Regel 7; Aufruf `/skill-name` `[DOK]`). Skills sind versioniert, getestet und über `allowed-tools` und `permissions` technisch abgesichert; eine Prompt-Vorlage ist es nicht.
...
Jede Datei `leitwerk-core/prompts/NN-<name>.md` folgt derselben Struktur: Metadatenblock (ID `FW-PR-0NN`, Version, Status, Owner, Betriebsmodus, typische Kontrollstufe, verwandter Skill) und acht Abschnitte:
...
Vollständig in `leitwerk-core/framework/core/06-prompting-rules.md`; die Kurzform ersetzt das Dokument nicht.
...
3. **Referenzen statt Kopien.** Dateien per Pfad referenzieren; eingefügter Text MUSS vorher auf seine Kontextklasse geprüft sein (`leitwerk-core/decision-trees/01-context-allowed.md`, `leitwerk-core/checklists/02-privacy-context.md`).
...
- Jede Vorlage trägt Version (`MAJOR.MINOR.PATCH`) und Status (`entwurf`, `pilot`, `aktiv`, `veraltet`, `zurückgezogen`) analog zum Skill-Standard (`leitwerk-core/framework/core/08-skill-conventions.md`, Abschnitt 7). MAJOR: Änderung der Struktur, des Ausgabeformats oder des Scopes; MINOR: neue Schritte, Parameter oder Prüfungen ohne Formatbruch; PATCH: Korrekturen und Formulierungen.
- Änderungen an Vorlagen erfolgen über den Änderungsprozess des Frameworks (`leitwerk-core/framework/core/01-governance.md`); Projekte DÜRFEN Vorlagen in `leitwerk-core/prompts/` NICHT anpassen. Projektspezifische Ergänzungen gehören in das Project Overlay, nicht in dieses Verzeichnis (`leitwerk-core/decision-trees/06-rule-placement.md`).
- Die strukturelle Konformität (Platzhalter, verbotene Muster, Codeblöcke) prüft `leitwerk-core/tests/scripts/validate-framework.py`; die Wirksamkeit einer Vorlage wird wie bei Skills über Testsitzungen auf dem Übungsrepository bewertet. Systematische Befunde aus Reviews (dieselbe Auffälligkeit bei mehreren Einsätzen) werden als Feedback an `<FRAMEWORK_OWNER>` gemeldet.
