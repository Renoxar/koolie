...
def skillorte(baum):
    ...
    for art in ARTEN:
        basis = os.path.join(baum, "leitwerk-core", "framework", art)
        if not os.path.isdir(basis):
            continue
        for pack in sorted(os.listdir(basis)):
            if os.path.isdir(os.path.join(basis, pack, "skills")):
                orte.append(os.path.join("leitwerk-core", "framework", art, pack,
                                         "skills"))
    return orte
...
def skillschnitt(baum):
    ...
    # Der Stammwaechter: ein Praefixvergleich auf `.claude/` haette `leitwerk-core/`
...
def _installer(baum):
    ...
    """
    pfad = os.path.join(baum, "leitwerk-core", "install.py")
...
def manifest(baum, client):
    ...
    pfad = os.path.join(baum, "leitwerk-core", "clients", client, "manifest.json")
    if not os.path.isfile(pfad):
...
def packverzeichnis(baum, pack):
    ...
    for art in ARTEN:
        p = os.path.join(baum, "leitwerk-core", "framework", art, pack)
        if os.path.isdir(p):
            return art, p
...
def entfernen(baum, client, pack, mit_quelle=True):
    ...
    # Ein Praefixvergleich auf der Laufzeitablage haette `leitwerk-core/` nie
