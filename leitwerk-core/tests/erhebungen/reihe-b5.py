...
"""Faehrt eine Reihe von Laeufen nacheinander, JE LAUF IM EIGENEN BAUM.
    ...
    python reihe-b5.py                      # alle 30
    python reihe-b5.py sk010p01 sk010p02    # ausgewaehlte

Der Baum eines Laufs heisst wie der Lauf: C:\\lw-b5\\<kennung>. Buendel 5 misst
einen Skill, der NICHT schreibt - der eigene Baum je Lauf hat hier einen anderen
Grund: Zwei Zellen brauchen eine je Meszbaum gesetzte Praeparation, und zwei
weitere brauchen ihr Gegenteil (D-240).
...
def main():
    ...
    if not kennungen:
        ...
        # eine Auflistung des Promptverzeichnisses. Der Nachlauf von Buendel 5 hat
        ...
        if not namen:
            raise SystemExit(
                ...
                "fahren. Erst `umgebungen-bauen-b5.py`, dann `baeume-b5.py` "
                "(D-230)." % BASIS)
    ...
    for k in kennungen:
        ...
        if not os.path.isdir(baum):
            raise SystemExit("ABBRUCH: der Baum %s fehlt - erst baeume-b5.py" % baum)
        abbruch = False
