...
# bezeichnen LEITWERK-CORE/VERSION und leitwerk-core/VERSION dieselbe Datei, und der Hook
...
STRUCTURE_PATH_PATTERNS = [
    ...
    re.compile(r"(^|[\\/])project-overlay[\\/]", re.I),
...
PROJEKTWURZEL = os.path.realpath(os.path.dirname(os.path.dirname(
...
PROTECTED_WRITE_PATH_PATTERNS = [
    re.compile(r"(^|[\\/])" + re.escape(CORE_DIR_NAME) + r"[\\/]", re.I),
]
...
def unpruefbar(grund: str) -> None:
    ...
    if fail_closed():
        block(f"Framework-Regel: Der Schutz-Hook kann diese Werkzeugeingabe nicht "
              ...
              f"({CORE_DIR_NAME}/governance/CHANGE_REQUEST_TEMPLATE.md).")
...
def aufgeloestes_material(werte, basis: str) -> tuple:
    ...
    Secret, ein 'project-overlay/' ausserhalb der Projektwurzel ist nicht unseres.
    """
    voll, nur_secret = [], []
    ...
    for label, pattern in SECRET_PATTERNS:
        if any(pattern.search(s) for s in strings):
            block(f"Framework-Regel: Werkzeugeingabe enthaelt ein Muster der Kategorie '{label}'. "
                  ...
                  f"({CORE_DIR_NAME}/framework/core/02-privacy.md, Abschnitt 5).")
