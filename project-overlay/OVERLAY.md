...
<!-- AUSFÜLLHINWEISE (vor Aktivierung entfernen oder belassen – sie sind für Devin unschädlich):
     ...
     - Das Overlay darf Framework-Regeln konkretisieren oder verschärfen, nie lockern
       (governance/PRIORITY_HIERARCHY.md).
     - Nach jeder Änderung: python3 tests/scripts/validate-framework.py ausführen und die Laufzeitfassung
...
| Overlay-Status | `<TBD: aktiv / inaktiv>` | `aktiv` erst nach Abschluss der Checkliste `checklists/10-project-adoption.md` |
| Overlay-Version | `<TBD: 0.1.0>` | Semantic Versioning, unabhängig von der Framework-Version |
| Kompatible Framework-Version | `<TBD: z. B. 0.1.x>` | aus `VERSION` des Frameworks |
...
| Ergebnis Datenschutz- und Vertragsprüfung | `<TBD: Referenz oder „ausstehend">` | siehe `framework/core/02-privacy.md` Abschnitt 1 |
...
├── framework/                     # Framework Core, Packs (nicht ändern)
...
| Ausgeschlossene Pfade (weder lesen noch ändern) | `<EXCLUDED_PATHS>` | `<TBD: z. B. deploy/**, infra/**, config/prod/**, **/fixtures/real/**>` | zusätzlich zu den festen Framework-Ausschlüssen (Secrets, `.devin/`, `AGENTS.md`, `project-overlay/`, `framework/`) |
...
| Merge-Request-Vorlage | `<MR_TEMPLATE_PATH>` | `<TBD>`; enthält den Devin-Nutzungsvermerk (`templates/MR_AI_DISCLOSURE.md`) |
...
Zusätzlich zur projektweiten Definition of Done (`<TBD: Pfad>`) gilt `framework/core/04-quality.md` Abschnitt 3 (Ergebnisbericht, lokale Prüfungen, Selbstreview, Nutzungsvermerk, offene Punkte sichtbar). Projektspezifische Ergänzungen: `<TBD>`.
...
Zusätzlich zu den festen K3-Kategorien (`framework/core/02-privacy.md` Abschnitt 2.1):
...
| Reviewerinnen und Reviewer | – | Review nach `framework/core/07-review-rules.md` |
...
| Stufe (aus `framework/core/10-error-escalation.md`) | Erreichbar über | Reaktionszeit (projektspezifisch) |
|---|---|---|
...
| E4 Framework-Mangel | `<FRAMEWORK_OWNER>` über `governance/FEEDBACK_PROCESS.md` | `<TBD>` |
...
Regeln: Projekt-Skills folgen dem Skill-Standard (`framework/core/08-skill-conventions.md`), tragen das Präfix `prj-`, liegen unter `.devin/skills/prj-<name>/` und DÜRFEN Framework-Skills ergänzen, aber deren Prüfschritte nicht entfernen.
...
Ausnahmen folgen `governance/EXCEPTION_PROCESS.md`. Ausnahmen von Delegationsverboten (V1–V12) und von der K3-Definition sind nicht zulässig.
...
Der Status wird erst auf `aktiv` gesetzt, wenn `checklists/10-project-adoption.md` vollständig abgearbeitet ist und `python3 tests/scripts/validate-framework.py --strict-overlay` ohne Befund durchläuft.
