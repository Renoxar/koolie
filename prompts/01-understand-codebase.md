...
- K3 gemäß `framework/core/02-privacy.md`: Secrets, `.env`-Werte, personenbezogene Echtdaten, Produktionsdaten, interne Adressen.
...
- K2-Inhalte ohne dokumentierte Freigabe nach `framework/core/02-privacy.md` Abschnitt 4.
...
| `{kontrollstufe}` | MUSS | K1 | niedrig, mittel oder hoch aus dem Preflight (`checklists/01-preflight.md`) |
| `{faktor}` | MUSS | K1 | Auslösender Risikofaktor R1–R13 gemäß `framework/core/09-risk-model.md` |
...
Betriebsmodus: M1 Read-only Analysis (framework/core/05-working-model.md). Du erzeugst, änderst, verschiebst oder löschst keine Datei und führst keine Befehle aus.
...
Ausgabeformat: Repository-Analyse nach .devin/skills/fw-repo-analyze/SKILL.md Abschnitt 5, ergänzt um den Abschnitt „Antworten auf den Fragenkatalog"; abschließend der Ergebnisbericht nach framework/core/05-working-model.md Abschnitt 3.6.
...
- Ergebnisbericht nach `framework/core/05-working-model.md` Abschnitt 3.6 (keine Änderungen, keine Befehle).

## 7. Prüfschritte

- [ ] Mindestens drei Fundstellen geöffnet und den jeweiligen Befund bestätigt (`framework/core/00-principles.md`, P4).
...
- [ ] Keine zitierten Konfigurationswerte, Secrets oder personenbezogenen Daten im Bericht (`checklists/02-privacy-context.md`).
- [ ] Ausgeschlossene Pfade wurden nicht gelesen; der Bericht benennt sie als ausgeschlossen.
- [ ] Offene Fragen an das Team mit der Mentorin oder dem Mentor besprochen; Auffälligkeiten nicht ungeprüft weitergegeben (`checklists/09-onboarding.md`).
