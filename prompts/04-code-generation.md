...
- Bestätigter Plan nach `templates/PLAN_TEMPLATE.md` oder bestätigte Schrittfolge mit Bestätigungsvermerk (Rolle, Datum, Referenz) (K1).
...
- K3 gemäß `framework/core/02-privacy.md`; `<EXCLUDED_PATHS>`; `<READ_ONLY_PATHS>` als Änderungsziel; Werte aus Konfigurations- und Umgebungsdateien.
...
Betriebsmodus: M3 Controlled Modification (framework/core/05-working-model.md). Schreib- und Ausführungsanfragen bestätige ich einzeln; du arbeitest ausschließlich auf Basis des bestätigten Plans.
...
Ausgabeformat: Änderungssatz mit Schrittprotokoll (Schritt, Dateien, Befehl, Ergebnis), Nachweis der API-Existenz, Abweichungen vom Plan, Commit-Vorschlag nach <COMMIT_CONVENTION>; abschließend der Ergebnisbericht nach framework/core/05-working-model.md Abschnitt 3.6.
...
- Ergebnisbericht nach `framework/core/05-working-model.md` Abschnitt 3.6.

## 7. Prüfschritte

- [ ] `checklists/03-before-code-change.md` vor dem ersten Schritt abgearbeitet; Plan bestätigt, bei Stufe hoch Freigabe und Pairing dokumentiert.
...
- [ ] Sicherheit und Datenschutz im neuen Code-Pfad geprüft: Eingabevalidierung, Autorisierung, Logging, Fehlermeldungen (RV7, RV8; `checklists/06-security.md`).
- [ ] `<TEST_COMMAND>` und `<LINT_COMMAND>` selbst ausgeführt; ab Stufe mittel zusätzlich durch die Reviewerin oder den Reviewer (`checklists/05-testing.md`).
- [ ] `checklists/04-review-ai-code.md` abgearbeitet; Merge Request mit Devin-Nutzungsvermerk (`templates/MR_AI_DISCLOSURE.md`, `checklists/08-merge-request.md`).
...
| „Nutze eine passende Bibliothek dafür" | Neue Abhängigkeit ohne Prüfung (V3, T4); halluzinierte Pakete möglich | Optionsanalyse anfordern; Entscheidung nach `checklists/07-new-dependency.md` |
