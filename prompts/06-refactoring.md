...
- Bestätigter Plan nach `templates/PLAN_TEMPLATE.md` ab Stufe mittel; Freigabereferenz bei Stufe hoch (K1).
...
Betriebsmodus: M3 Controlled Modification (framework/core/05-working-model.md). Schreib- und Ausführungsanfragen bestätige ich einzeln.
...
Ausgabeformat: Refactoring-Protokoll nach .devin/skills/fw-refactor/SKILL.md Abschnitt 5; abschließend der Ergebnisbericht nach framework/core/05-working-model.md Abschnitt 3.6.
...
- Gemeldete, nicht geänderte Befunde mit Fundstelle und empfohlenem Folge-Skill; Commit-Vorschläge je Schritt nach `<COMMIT_CONVENTION>`; Annahmen und offene Fragen; Ergebnisbericht nach `framework/core/05-working-model.md` Abschnitt 3.6.

## 7. Prüfschritte

- [ ] `checklists/03-before-code-change.md` vor dem ersten Schritt abgearbeitet; ab Stufe mittel Plan bestätigt, bei Stufe hoch Freigabe und Pairing dokumentiert.
...
- [ ] Testnachweis vorher und nach jedem Schritt nachvollzogen; `<TEST_COMMAND>` selbst ausgeführt; ab Stufe mittel durch die Reviewerin oder den Reviewer (`checklists/05-testing.md`).
...
- [ ] Ein Commit je Schritt; `checklists/04-review-ai-code.md` abgearbeitet; Merge Request mit Devin-Nutzungsvermerk (`checklists/08-merge-request.md`).

## 8. Typische Fehlanwendungen

| Fehlanwendung | Folge | Stattdessen |
|---|---|---|
| „Räum das Modul auf" ohne Ziel und Invarianten | Unkontrollierter Scope (`framework/core/06-prompting-rules.md`, Regel 3); nicht prüfbarer Diff | Refactoring-Ziel und `{unveraendert}` konkret benennen; Umfang unter `<CHANGE_SIZE_THRESHOLD>` |
