...
Betriebsmodus: M5 Documentation Support (framework/core/05-working-model.md): Schreiben nur in <DOC_PATHS>; kein Quellcode, keine Konfiguration; Befehle nur lesende Git-Befehle.
...
- [ ] Prüfung auf vertrauliche Inhalte vor Übernahme oder Ablage in `<DOCUMENTATION_PLATFORM>` (`checklists/02-privacy-context.md`).
- [ ] Konventionen und Glossarbegriffe eingehalten; Beispiele als synthetisch gekennzeichnet.
- [ ] Review über den regulären Prozess (`checklists/08-merge-request.md`), Devin-Nutzungsvermerk enthalten.
