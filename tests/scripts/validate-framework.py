...
Aufruf (im Wurzelverzeichnis des Repositorys):
    python3 tests/scripts/validate-framework.py [--strict-overlay] [--mermaid] [--root PFAD]
...
Prüft (statisch, ohne Devin):
  ...
  6. Verbotene Inhalte: Secret-Muster, E-Mail-Adressen, IP-Adressen, interne Hostnamen,
     URLs außerhalb der Quellen-Allowlist, projektspezifische Sperrbegriffe (tests/forbidden-terms.txt)
  7. Platzhalter: nur registrierte Platzhalter (docs/PLACEHOLDER_REGISTRY.md)
...
(Widerspruchsfreiheit, Verhalten von Devin) erfolgt über tests/TEST_CATALOG.md.
"""
...
REQUIRED_PATHS = [
    "AGENTS.md", "README.md", "CHANGELOG.md", "VERSION", "OWNERS.md",
    ...
    "framework/core/00-principles.md", "framework/core/02-privacy.md", "framework/core/03-security.md",
    "framework/core/05-working-model.md", "framework/core/08-skill-conventions.md",
    "framework/core/09-risk-model.md", "framework/role-packs", "framework/tech-packs",
    "project-overlay/OVERLAY.md", "project-overlay/overlay-manifest.yaml",
    "templates/SKILL_TEMPLATE.md", "prompts", "checklists", "decision-trees", "onboarding",
    "examples", "tests/TEST_CATALOG.md", "governance/RACI.md", "governance/DECISION_LOG.md",
...
def load_placeholder_registry(root: str) -> set[str]:
    path = os.path.join(root, "docs", "PLACEHOLDER_REGISTRY.md")
    if not os.path.exists(path):
        return set()
...
def check_content(root: str) -> None:
    ...
    ft_path = os.path.join(root, "tests", "forbidden-terms.txt")
    ...
    for path in iter_text_files(root):
        ...
        if rel.startswith("tests/scripts/") or rel == "tests/forbidden-terms.txt":
            continue
        text = read(path)
        ...
        if FENCE4_RE.search(text) and rel.startswith((".devin/", "framework/", "prompts/", "checklists/",
                                                       "decision-trees/", "onboarding/", "templates/",
                                                       "project-overlay/", "governance/", "pilot/")):
    ...
    for ph, files in sorted(unknown_placeholders.items()):
        warn(f"Platzhalter <{ph}> nicht in docs/PLACEHOLDER_REGISTRY.md (in {', '.join(sorted(files)[:3])}{'…' if len(files) > 3 else ''})")
